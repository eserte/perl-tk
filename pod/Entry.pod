...
I<$entry> = I<$parent>-E<gt>B<Entry>(?I<options>?);
...
$widget argument) and makes it into an entry widget.
...
(see the B<exportSelection> option), then it will observe the standard X11
protocols for handling the selection;  entry selections are available as type
B<STRING>. Entries also observe the standard Tk rules for dealing with the
...
=item I<$entry>-E<gt>B<xview>(B<moveto> =E<gt> I<fraction>)
...
=item I<$entry>-E<gt>B<xview>(B<scroll> =E<gt> I<number, what>)
