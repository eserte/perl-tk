...
styles can be used. For example, suppose you want to display two
...
name of a B<imagetext> display style that has already be created by
the B<tixDisplayStyle(n)> command.
...
name of a B<text> display style that has already be created by the
B<tixDisplayStyle(n)> command.
...
Display items of the type I<$widget> are used to display a
...
name of a $widget display style that has already be created by
the B<tixDisplayStyle(n)> command.
...
example, the HList widgets has a command B<item> which can be used
...
The B<item create> command of the HList widget accepts a variable
...
the HList widget has the command B<item configure>, B<item cget>
and B<item delete> for accessing the display items.

=head1 CREATING AND MANIPULATING DISPLAY STYLES

Display styles are created by the command B<tixDisplayStyle>:

=head1 SYNOPSIS

B<tixDisplayStyle>I< >I<itemType> ?I<-stylename name>? ?I<-refwindow pathName>? ?I<options value ...>?

I<itemType> must be one of the existing display items types such as
B<text>, B<imagetext>, $widget or any new types added by
...
=item B<-refwindow> =E<gt> I<$widget>
...
The B<tixDisplayStyle> command creates a widget object whose name is the
...
=item I<styleName option >?I<arg arg ...>?

I<styleName> is the name of the command. I<Option> and the
I<arg>s determine the exact behavior of the command. The following
commands are possible:

=item I<styleName >B<cget> I<option>

Returns the current value of the configuration option given by
I<option>. I<Option> may have any of the valid options of this
display style.

=item I<styleName >B<configure> ?I<option>? I<?value option value ...>?
...
available options for I<styleName> (see L<Tk::configure> for
...
the command returns an empty string. I<Option> may have any of the
valid options of this display style.

=item I<styleName >B<delete>
...
that the income data is aligned properly.
   ...
   #Tried: tixDisplay[Ss]tyle, Display[Ss]tyle, DItem all fail :-(
   #my $red  = $hlist->DStyle('text', -foreground=>'#800000');
   #my $blue = $hlist->DStyle('text', -foreground=>'#000080');
