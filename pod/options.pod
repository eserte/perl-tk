...
Tk::options - Standard options supported by widgets

=for category Creating and Configuring Widgets

=head1 SYNOPSIS

I<$var> = I<$widget>-E<gt>B<cget>('I<option>');

I<$widget>-E<gt>B<configure>(I<option> =E<gt> I<value >?,I<option> =E<gt> I<value ...>?);

I<@var> = I<$widget>-E<gt>B<configure>('I<option>');

I<@var> = I<$widget>-E<gt>B<configure>;
...
=item <$widget>-E<gt>B<configure>(-I<option> =E<gt> I<value >?,I<option> =E<gt> I<value ...>?);
...
In array context returns a list of five elements:

=over 8

=item I<Configure Option>

The value of I<option>.

=item I<Name>

The option's name in the option database (e.g.
in F<.Xdefaults> files, see L<Tk::option>).
...
The current value (as returned by B<cget>).
...
database (e.g. in .Xdefaults files).  ``Class'' refers to the option's class value
in the option database.  ``Option'' refers to the switch used in widget-creation
and B<configure> widget methods to set this value. For example, if an option's
configure option is B<-foreground> and there exists a widget $widget, then the
call:

 $widget->configure(-foreground => 'black')

may be used to specify the value B<black> for the option in the widget $widget.
