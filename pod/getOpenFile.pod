...
B<tk_getOpenFile >?I<option value ...>?

B<tk_getSaveFile >?I<option value ...>?

=head1 DESCRIPTION

The procedures B<tk_getOpenFile> and B<tk_getSaveFile> pop up a
dialog box for the user to select a file to open or save. The
B<tk_getOpenFile> command is usually associated with the B<Open>
...
The B<tk_getSaveFile> command is usually associated with the B<Save>
...
=item B<-defaultextension> I<extension>
...
=item B<-filetypes> I<filePatternList>
...
discussion on the contents of I<filePatternList>.

=item B<-initialdir> I<directory>
...
=item B<-initialfile> I<filename>

Specifies a filename to be displayed in the dialog when it pops
up. This option is ignored by the B<tk_getOpenFile> command.

=item B<-parent> $widget

Makes $widget the logical parent of the file dialog. The file
...
If the user selects a file, both B<tk_getOpenFile> and
B<tk_getSaveFile> return the full pathname of this file. If the
...
The I<filePatternList> value given by the B<-filetypes> option
