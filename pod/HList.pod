...
B<-background>	B<-borderwidth>	B<-cursor>	B<-foreground>
...
Specifies a perl/Tk B<callback> to be executed when the user manipulates the
...
Specifies a perl/Tk B<callback>B< to be called whenever the HList widget>
...
be displayed for this list entry. Please see the L<Tk::DItem> documentation
page for a list of all display items.
When a list entry is created by the B<add> or B<addchild> widget
commands, the type of its display item is determined by the
B<-itemtype> option passed to these commands. If the
...
=item I<$hlist>-E<gt>B<add>(I<entryPath, >?I<option, value, ...>?)
...
item given by the B<-itemtype> option. Please see the
B<DItem(n)> manual page for a list of the configuration options for
...
entryPath of the new entry will be B<foo.1>, B<foo.2>, ... etc.
...
I<width> must be in a form accepted by B<Tk_GetPixels(3)>.
...
=item I<$hlist>-E<gt>B<entrycget>(I<entryPath, option>)
...
=item I<$hlist>-E<gt>B<entryconfigure>(I<entryPath, >?I<option>?, I<?value, option, value, ...>?)
...
I<option> is specified with no I<value>, then the command
...
=item I<$widget>-E<gt>B<header>(I<option, >I<col, >?I<args, ...>?)
...
=item I<$widget>-E<gt>B<header>(B<cget, >I<col, >I<option>)
...
=item I<$widget>-E<gt>B<header>(B<configure, >I<col, >?I<option>?, I<?value, option, value, ...>?)
...
=item I<$widget>-E<gt>B<header>(B<create, >I<col, >?)I<-itemtype type>? ?I<option value ...>?
...
=item I<$widget>-E<gt>B<indicator>(I<option, >entryPath, ?I<args, ...>?)
...
=item I<$widget>-E<gt>B<indicator>(B<cget, >I<entryPath, >I<option>)
...
=item I<$widget>-E<gt>B<indicator>(B<configure, >I<entryPath, >?I<option>?, I<?value, option, value, ...>?)
...
I<value>, then the command returns a list describing the one named
...
I<option-value> pairs are specified, then the command modifies the
...
=item I<$widget>-E<gt>B<indicator>(B<create, >I<entryPath, >?)I<-itemtype type>? ?I<option value ...>?
...
=item I<$widget>-E<gt>B<info>(I<option, >I<arg, ...>)
...
=item I<$widget>-E<gt>B<info>(B<children, >?I<entryPath>?)

If I<entrpyPath> is given, returns a list of the entryPath's of its
children entries. Otherwise returns a list of the toplevel
entryPath's.

=item I<$widget>-E<gt>B<info>(B<data, >?I<entryPath>?)

Returns the data associated with I<entryPath>.

=item I<$widget>-E<gt>B<info>(B<dragsite>)

I<$widget>-E<gt>B<info>(B<dropsite>)
...
I<entrpyPath>. If I<entrpyPath> is a toplevel list entry,
...
=item I<$hlist>-E<gt>B<itemCget>(entryPath, col, option)
...
=item I<$hlist>-E<gt>B<itemConfigure>( entryPath, col, ?I<option>?, I<?value, option, value, ...>?)
...
=item I<$hlist>-E<gt>B<itemCreate>(entryPath, col, ?I<-itemtype type>? ?I<option value ...>?)
...
=item I<$widget>-E<gt>B<selectionGet>
...
=item I<$hlist>-E<gt>B<show>(I<option, ?entryPath?>)

Show the entries that are hidden by the B<hide> command,
I<option> can be one of the following:
