...
B<-highlightbackground>	B<-insertofftime>	B<-selectborderwidth>
B<-highlightcolor>	B<-insertontime>	B<-selectforeground>
...
any (see the B<focus> method below for more on this).  If a
...
=item B<-colormap> =E<gt> I<varName>
...
=item B<-fontmap> =E<gt> I<varName>

I<VarName> must be the name of an array variable
that specifies a font mapping to use in the Postscript.
Each element of I<varName> must consist of a array with
...
=item I<$canvas>-E<gt>B<scanDragto>(I<x, y>.)
...
=item I<$canvas>-E<gt>B<yview>(B<moveto> =E<gt> I<fraction>)
...
=item I<$canvas>-E<gt>B<yview>(B<scroll> =E<gt> I<number, what>)
...
the arc's region, in any of the forms described in
