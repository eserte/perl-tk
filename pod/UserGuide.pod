...
like described in the L<Encode> manpage, then Perl/Tk will very
probably display and handle this string correctly.

Note that every variable which is passed somehow into a Perl/Tk method
will be implicitely changed into a internally utf8-flagged variable.
Semantically nothing changes (the series of codepoints stays the
same), but things will change if variables with high-bit iso-8859-1
...
For data which comes from an file you have to specify the encoding
...
Likewise, for data which is read from Tk widgets and should be output
into a file, the encoding has to be specified. In the output case the
encoding should be always specified, even if it should be written as
iso-8859-1:
...
layer, as described above. For other cases, for example dealing with
databases, most often the encoding has to be done manually, unless the
database driver has some means for automatically do this for you. So
when working with a MySQL database, one could use:
...
Unfortunately, there are still places in Perl where Unicode does not
happen. One of these places are filenames. Consequently, the file
selectors in Perl/Tk do not handle encoding of filenames properly.
Currently they suppose that filenames are in iso-8859-1 encoding, at
least on Unix systems. As soon as Perl has a concept of filename
encodings, then Perl/Tk will also implement such schemes.
