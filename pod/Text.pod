...
For example, B<-tabs =E<gt> "2c left 4c 6c center"> creates three
...
an empty list, such as B<-tags => " ">, then it requests
default 8-character tabs as described for the B<tags>
widget option.
...
when the image is inserted into the text widget widh B<image create>.
...
B<image create> method, several configuration
options may be associated with it.
...
=item B<-command> =E<gt> I<command>

Instead of returning the information as the result of the dump operation,
invoke the I<command> on each element of the text widget within the range.
...
The behavior of the command depends on the I<option> argument
that follows the B<tag> argument.
...
(see L<Tk::configure> for information on the format of this list).
...
the text using the rules for regular expressions
(see the B<regexp> command for details).
...
the most specific binding is chosen
(see the L<Tk::bind> documentation for details).
...
=item I<$text>-E<gt>B<xview>(B<moveto> =E<gt> I<fraction>)
...
=item I<$text>-E<gt>B<xview>(B<scroll> =E<gt> I<number, what>)
...
=item I<$text>-E<gt>B<yview>(B<moveto> =E<gt> I<fraction>)
...
=item I<$text>-E<gt>B<yview>(B<scroll> =E<gt> I<number, what>)
...
Set the B<insertOffTime> attribute to 0 avoid this.
