...
I<$toplevel>-E<gt>I<method>(?I<args>?)
...
If I<name> is specified, this command stores I<name> (which
...
set in a B<client> command for I<$toplevel>.
...
This command is used to manipulate the B<WM_COLORMAP_WINDOWS>
property, which provides information to the window managers about
windows that have private colormaps.
If I<windowList> isn't specified, the command returns a list
...
the command overwrites the B<WM_COLORMAP_WINDOWS>
...
If I<value> is specified, this command stores I<value> in I<$toplevel>'s
...
set in a B<command> command for I<$toplevel>.
...
to the command, then it specifies the focus model for I<$toplevel>.
...
The focus model defaults to B<passive>, and Tk's B<focus> command
...
in a B<geometry> command).  I<NewGeometry> has
...
=item I<$toplevel>-E<gt>B<grid>(?I<baseWidth,baseHeight,widthInc,heightInc>?)

This command indicates that I<$toplevel> is to be managed as a
gridded window.
...
I<$toplevel>'s icon, and the command returns an empty string.  If
an empty string is specified for I<bitmap>, then any current icon
bitmap is cancelled for I<$toplevel>.
...
then the command returns the current icon name for I<$toplevel>,
or an empty string if no icon name has been specified (in this
case the window manager will normally display the window's title,
as specified with the B<title> command).
...
If neither I<x> nor I<y> is specified, then the command returns
...
If resizing has been disabled with the B<resizable> command,
...
when a B<geometry> command is invoked, unless the source has
...
If I<width> and I<height> are omitted then the command
...
to a window that has been iconified (e.g., with the B<wm iconify>
command) while B<icon> refers to a window whose only purpose is
to serve as the icon for some other window (via the B<wm iconwindow>
command).
...
top-level window using the B<geometry> command.
...
However, you can use the B<minsize> and B<maxsize> commands
...
well as manual resizes and the B<geometry> command.
You can also use the command B<resizable> to completely
...
B<geometry> command or by interactively resizing the window.
...
with the B<grid> command or by calling B<Tk_SetGrid>.
...
in B<minsize>, B<maxsize>, and B<geometry> commands
