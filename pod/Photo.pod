...
B<image create photo >?I<name>? ?I<options>?
...
Like all images, photos are created using the B<image create>
command.
Photos support the following I<options>:
...
command copies the whole of I<sourceImage> into I<$image>,
...
format as accepted by I<$image>B<put>?.
...
=item I<$image>-E<gt>B<put>(I<data> ?B<-format>I< format-name>? ?B<-to>I< x1 y1 x2 y2>?)
...
Otherwise I<data> is used to form a two-dimensional array of pixels
...
B<-to> option can be used to specify the area of I<$image> to be
affected.  If only I<x1> and I<y1> are given, the area affected
has its top-left corner at (I<x1,y1>) and is the same size as the
array given in I<data>.  If all four coordinates are given, they
specify diagonally opposite corners of the affected rectangle, and the
array given in I<data> will be replicated as necessary in the X and
