...
(See L<Tk::configure> for a description of this behaviour.)
...
a hash table for the widget. (More than one key =>
value pair can be specified to a single call.)
...
apply B<configure> to I<advertised> subwidgets (see L<ptkAdvertised>).
