...
sub floor {
    ...
    $w_vscroll->configure(-command => [$c => 'yview']);
    $w_hscroll->configure(-command => [$c => 'xview']);
    ...
    my %floor_items = ();
    my $active_floor = 0;
    floor_display $c, 3, \%floor_labels, \%floor_items, \%cinfo,
        \$active_floor, $c_entry;
...
sub floor_display {
    ...
    my($w, $active, $floor_labels, $floor_items, $cinfo, $active_floor,
       $c_entry) = @ARG;
    ...
    floor_bg1 $w, $cinfo->{'floor_bg1'}, $cinfo->{'outline1'};
    floor_bg2 $w, $cinfo->{'floor_bg2'}, $cinfo->{'outline2'};
    floor_bg3 $w, $cinfo->{'floor_bg3'}, $cinfo->{'outline3'};
    ...
    $w->create('window', 600, 100, -anchor => 'w', -window => $c_entry);
