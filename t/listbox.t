...
#XXX not yet in 800.022     [qw{-listvar}, \$testVariable,  testVariable {}}, q{}],
	...
	if ($test->[3] ne "") {
	    ...
	    like($@,qr/$test->[4]/,"wrong error message for $name");
	}
...
if ($Listbox ne 'Listbox') {
    ...
    eval { Tk::listbox() };
    like($@,qr/Usage \$widget->listbox(...)/, "wrong error message");
}
...
eval { $lb->bbox("fooey") };
like($@,qr/bad listbox index "fooey": must be active, anchor, end, \@x,y, or a number/, "wrong error message");
...
$lb->yview(3);
$lb->update;
is(join(" ", $lb->bbox(3)), "7 7 17 14");
is(scalar @{[$lb->bbox(3)]}, 4);
is(($lb->bbox(3))[0], 7);
is(($lb->bbox(3))[-1], 14);
is(join(" ", $lb->bbox(4)), "7 26 17 14");

$lb->yview(0);
$lb->update;
is($lb->bbox(-1), undef);
is(join(" ", $lb->bbox(0)), "7 7 17 14");

$lb->yview("end");
$lb->update;
is(join(" ", $lb->bbox(17)), "7 83 24 14");
is(join(" ", $lb->bbox("end")), "7 83 24 14");
is($lb->bbox(18), undef);

{
    ...
    $lb->update;
    $lb->xview(moveto => 0.2);
    is(join(" ", $lb->bbox(2)), '-72 39 393 14');
    $t->destroy;
}

mkPartial();
is(join(" ", $partial_lb->bbox(3)), "5 56 24 14");
is(join(" ", $partial_lb->bbox(4)), "5 73 23 14");
...
eval { $lb->cget(-gorp) };
like($@,qr/unknown option "-gorp"/, "wrong error message");

is($lb->cget(-setgrid), 0);
# XXX why 25 in Tk800?
is(scalar @{[$lb->configure]}, ($Tk::VERSION < 803 ? 25 : 27));
is(join(" ", $lb->configure(-setgrid)),
...
$lb->selection("set", 9);
is(join(" ", $lb->curselection), "3 4 5 6 9");
...

{
    ...
    $l2->insert(0, qw(el0 el1 el2 el3 el4 el5 el6 el7));
    $l2->delete(-3, 2);
    is(join(" ", $l2->get(0, "end")), "el3 el4 el5 el6 el7");
    $l2->destroy;
}

{
    ...
    $l2->insert(0, qw(el0 el1 el2 el3 el4 el5 el6 el7));
    $l2->delete(-3, -1);
    is(join(" ", $l2->get(0, "end")), join(" ", map { "el$_" } (0 .. 7)));
...

{
    ...
    $l2->insert(0, qw(el0 el1 el2 el3 el4 el5 el6 el7));
    $l2->delete(5, 20);
    is(join(" ", $l2->get(0, "end")), "el0 el1 el2 el3 el4");
...

{
    ...
    $l2->insert(0, qw(el0 el1 el2 el3 el4 el5 el6 el7));
    $l2->delete("end", 20);
    is(join(" ", $l2->get(0, "end")), "el0 el1 el2 el3 el4 el5 el6");
...

{
    ...
    $l2->insert(0, qw(el0 el1 el2 el3 el4 el5 el6 el7));
    $l2->delete(8, 20);
    is(join(" ", $l2->get(0, "end")), "el0 el1 el2 el3 el4 el5 el6 el7");
    is(scalar @{[$l2->get(0, "end")]}, 8);
    $l2->destroy;
}
...
# XXX is in perl/Tk
#  eval { $lb->get("2.4") };
#  like($@ ,qr/bad listbox index "2.4": must be active, anchor, end, \@x,y, or a number/,
#     "wrong error message");
...

{
    ...
    is(join(" ", $l2->get(3, "end")), "two words el4 el5 el6 el7");
}

is($lb->get(-1), undef);
is($lb->get(-2, -1), undef);
is(join(" ", $lb->get(-2, 3)), "el0 el1 el2 el3");
is(scalar @{[ $lb->get(-2, 3) ]}, 4);

is(join(" ", $lb->get(12, "end")), "el12 el13 el14 el15 el16 el17");
is(scalar @{[ $lb->get(12, "end") ]}, 6);
is(join(" ", $lb->get(12, 20)), "el12 el13 el14 el15 el16 el17");
...

{
    ...
    is(join(" ", $l2->get(0, "end")), 'a b c x y z d e');
    is(scalar @{[ $l2->get(0, "end") ]}, 8);
    $l2->destroy;
}
...

{
    ...
    is(scalar @{[ $l2->get(0, "end") ]}, 4);
    $l2->insert(4, qw/y/);
    is(join(" ", $l2->get(0, "end")), 'a b c x y');
    $l2->insert(6, qw/z/);
    is(join(" ", $l2->get(0, "end")), 'a b c x y z');
    $l2->destroy;
}
...
like($@ ,qr/\'bogus\' isn\'t numeric/, "wrong error message");

## is in perl
#  eval { $lb->scan(qw/foo 2 2.3/) };
#  like($@ ,qr/'2.3' isn't numeric/,
#     "wrong error message");
...

{
    ...
    $lb->update;
    like(join(",",$lb->xview) ,qr/^0\.24936.*,0\.42748.*$/,
       join(",",$lb->xview));
...
is(join(",",$lb->curselection), "2,5,6,7,8");
...
is(join(" ", $lb->curselection), "2 5 6 7");
...
$lb->update;
is("@{[ $lb->xview ]}", '0.04 0.24');

$lb->xview(30);
$lb->xview(scroll => -1, "pages");
$lb->update;
is("@{[ $lb->xview ]}", '0.44 0.64');
...
is(join(" ",$lb->get(qw/0 end/)), 'q r s a b A c d x y z');
...
$top_lb->destroy;
like($top->geometry, qr/150x178\+\d+\+\d+/);
...
eval { $lb->index("a") };
like($@ ,qr/bad listbox index \"a\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("\@") };
like($@ ,qr/bad listbox index \"\@\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("\@foo") };
like($@ ,qr/bad listbox index \"\@foo\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("\@1x3") };
like($@ ,qr/bad listbox index \"\@1x3\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("\@1,") };
like($@ ,qr/bad listbox index \"\@1,\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("\@1,foo") };
like($@ ,qr/bad listbox index \"\@1,foo\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("\@1,2x") };
like($@ ,qr/bad listbox index \"\@1,2x\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");

eval { $lb->index("1xy") };
like($@ ,qr/bad listbox index \"1xy\": must be active, anchor, end, \@x,y, or a number/, "wrong error message $@");
...
$lb->update;
is("@{[ $lb->xview ]}", "0.2 0.4");
is(join(':',$lb->xview), "0.2:0.4");  # just to prove it is a list
is("@{[ $lb->yview ]}", "0.5 0.75");
is(join(':',$lb->yview), "0.5:0.75"); # just to prove it is a list
...
$lb->update;
is("@{[ $lb->xview ]}", "0.64 0.84");
is("@{[ $lb->yview ]}", "0.25 0.5");
...
is("@{[ $lb->curselection ]}", "2 3 8 9 10 11 12");
...
is(join(",",$lb->curselection), "0,1,2,3");

$lb->delete(qw/0 end/);
$lb->insert(qw/0 a b c d e f/);
$lb->selection(qw/clear 0 end/);
$lb->selection(qw/set 2 4/);
is(join(" ", $lb->curselection), "2 3 4");

$lb->delete(qw/0 end/);
$lb->insert(qw/0 a b c d e f/);
$lb->selection(qw/clear 0 end/);
$lb->selection(qw/set 4 end/);
is(join(" ", $lb->curselection), "4 5");
...
$e->selection(qw/from 0/);
$e->selection(qw/to 5/);
is(join(" ", $lb->curselection), "0 1 2 3 4");
...
$lb->insert(qw/0 foo/);
$lb->update;
like("@x" ,qr/Undefined subroutine &main::gorp called.*vertical scrolling command executed by listbox/s, "x is @x");
...
$lb->update;
is($log[0], "x 0 1");
like($log[1] ,qr/^x 0 0\.32258/, "expected: x 0 0.32258 in $log[1]");
is($log[2], "x 0 1");
...
$lb->insert(qw/0 foo/);
$lb->update;
like("@x" ,qr/Undefined subroutine &main::bogus.*horizontal scrolling command executed by listbox/s, "x is @x");
