...
sub arrows {
	...
	$demoArrowInfo{'boxStyle'} = [-fill => undef, -outline => 'black',
				      -width => 1];
	$demoArrowInfo{'activeStyle'} = [-fill => 'red', -outline => 'black'
					 , -width => 1];
    } else {
	$demoArrowInfo{'bigLineStyle'} = "-fill black
...
# The procedures below are called in response to mouse motion for one of the
# three items used to change the line width and arrowhead shape.  Each
# procedure updates one or more of the controlling parameters for the line
# and arrowhead, and recreates the display if that is needed.  The arguments
# are the name of the canvas widget, and the x and y positions of the mouse
# within the widget.
