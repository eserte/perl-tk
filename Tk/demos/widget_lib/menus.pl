...
sub menus {
    ...
    $w_menu = $w->Frame(-relief => 'raised', -bd => 2);
    ...
    $f->command(-label => 'Save As ...',
		-command => [\&error, 'Save As']);
    $f->separator;
    $f->command(-label => 'Print Setup ...',
		-command => [\&error, 'Print Setup']);
    $f->command(-label => 'Print ...',
		-command => [\&error, 'Print']);
    $f->separator;
    $f->command(-label => 'Quit', -command => ['destroy', $w]);
    ...
    $cc->separator;
    $cc->command(-label => 'Se current values',
    ...
    $rc->separator;
    $rc->command(-label => 'See current values',
    ...
    $i->command(
        -bitmap => "\@$tk_library/demos/images/pattern",
	-command => [sub {$DIALOG_ICON->show}],
    );
    foreach $label (qw(info questhead error)) {
	$i->command(
            -bitmap  => $label,
            -command => [sub {print "You invoked the $_[0] bitmap\n"},$label]
        );
    }
	...
	$m->command( 
            -label   => $label, 
	    -command => [sub {print "You invoked \"$_[0]\"\n"},$label]
        );
    }
