...
sub cscroll {
    ...
    my $w_vscroll = $w->Scrollbar(-command => ['yview', $c]);
    my $w_hscroll = $w->Scrollbar(-command =>
				  ['xview', $c], -orient => 'horiz');
    $c->configure(-xscrollcommand => ['set', $w_hscroll],
...
sub scroll_enter {
    ...
    $cscroll::old_fill = ($c->itemconfigure($id, -fill))[4];
    if ($CSCROLL->depth > 1) {
...
sub scroll_leave {
    ...
    my $id = $c->find('withtag', 'current');
    $id-- if ($c->gettags('current'))[0] eq 'text';
    $c->itemconfigure($id, -fill => $cscroll::old_fill);
