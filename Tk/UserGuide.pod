...
and put inside the GUI library.  This loop will watch all events of interest,
...
I advise also to use C<use strict> and use the B<-w> switch.
...
This is done by calling the a Tk constructor method on the variable.
...
=head1 Variables and call-back routines
...
Note the use of call-backs.  Note, also, that Tk handles the values, and
...
In the examples above, you must have noticed to C<pack> calls.
This is one of the more complicated parts of Tk.  The basic idea
is that any window or widget should be subject to a Tk window manager;
the I<packer> is the most useful window manager.
...
is some space left, or when the window size is increased.  For widgets
that you want to keep the same size, specify nothing; for widgets 
that you want to fill all space, specify B<-fill> to B<x>, B<y>, or B<both>;
for widgets that should grow, specify B<-expand =E<gt> 'yes'>.
These parameters are not shown in the example below; see the widget demo.
...
is independent; closing a toplevel window does not affect the others.
...
So far, all call-back routines shown called a user procedure.
You can also have a call-back routine call another Tk routine.
This is the way that scroll bars are implemented: scroll-bars
can call a Tk items, or a user procedure,  whenever their position
