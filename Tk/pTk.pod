...
languages. Currently tk4.0b2 runs under Perl using this
approach. Below I<Lang> is the notation for an external language to
which B<PortableTk> glues B<Tk> code.

The main problem with using the code developped for B<TCL> with
different language is the absence of data types: almost anything is
...
Currently B<PortableTk> uses also other, slightly different approach
...
need to be updated to use Language-independent functions looking on
compiler warnings.  Eventually a way to use these Language-independent
functions under proper B<TCL> will be also provided. End of this 
document provides a starting point for such a project.

=head1 Structure of B<pTk>, porting your code

B<pTk>, that is a port of B<Tk>, is very special with respect to port
...
automatic conversion by C<munge> script (written in Perl). Thus the
...
your code to B<portableTk> from the scratch, since this will make it
...
It is not guarantied that the C<args> can survive deletion of C<value>.
...
C<freeProc> C<LANG_DYNAMIC> (it is not guarantied that any change of
...
???? It is very antiintuitive, I hope the name is changed.
...
function are provided for manipulation of C<LangCallback>s:
...
takes not C<(char*)NULL>, but C<NULL> as delimiter. I<Autoconverted>.
...
=item Sequence C<Tcl_PrintDouble, Tcl_PrintDouble, ...,
Tcl_AppendResult> 
...
If you start with a working TCL code, you can start convertion using
...
below is mostly direct counterpart of API for construction of dynamic lists
in B<TCL>:
...
The difference is that a call to C<ListFactoryFinish> should preceed the
actual usage of the value of C<ListFactory>, and there are two
different ways to append an C<Arg> to a C<ListFactory>:
ListFactoryAppendCopy() guaranties that the value of C<arg> is copied
...
=head2 DStrings for production of strings

are still supported in B<portableTk>.
...
Use C<Tcl_ArgResult>. It is not guarantied that result survives this
