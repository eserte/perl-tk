/*
  Copyright (c) 2000 Nick Ing-Simmons. All rights reserved.
				...
				int * dstWrotePtr, int * dstCharsPtr)
{
 ...
 int chars = 0;
 dSP;
 SV *quiet = get_sv("Tk::encode_fallback",0);
 SV *stmp;
 SV *dtmp;
 char *td;
 STRLEN dbytes;
 ...
 if (s == send)
  {
   LangDebug(__FUNCTION__ "Nothing to convert\n");
  }
 while (s < send)
  {
#ifdef SvUTF8
...
#else
   I32 len;
   UV ch = utf8_to_uv(s,&len);
#endif
   if ((I32) len < 0)
    {
     LangDebug("%.*s => Syntax\n",srcLen,s);
     code = TCL_CONVERT_SYNTAX;
     break;
    }
#else
   I32 len = 1;
   ...
   XPUSHs(quiet);
   PUTBACK;
   perl_call_method("encode",G_SCALAR);
   SPAGAIN;
   dtmp = POPs;
   PUTBACK;
   if (SvCUR(stmp))
 ...
 *dstWrotePtr = (d- (U8 *)dst);
 if (0 && code != TCL_OK)
  {
   LangDebug("%s:%.*s => %d with %d bytes\n",Tcl_GetEncodingName(encoding),
             srcLen,src,code,*dstWrotePtr);
  }
 return code;
}
