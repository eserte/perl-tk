    ...
    Tcl_Interp *interp;
{
    ...
    if (stdin_opened ==  0)  {
	Tk_CreateFileHandler(Tcl_GetFile(0, TCL_UNIX_FD), TK_READABLE, StdinProc, (ClientData) interp);
	Prompt(interp, 0);
...
#define BUFFER_SIZE 4000
	...
	if (count <= 0) {
	    /* End of file */
	    Tk_CreateFileHandler(Tcl_GetFile(0, TCL_UNIX_FD), 0, StdinProc, (ClientData) interp);
	    return;
	}
    }
