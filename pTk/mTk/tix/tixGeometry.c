...
/*
 * This function is called when the clients initiates a geometry request
 * i.e., a button changes its text and now needs a larger width
 *
 */
static void
				 ...
				 * about the window. */
{
    ...
    result = Tcl_VarEval(cnPtr->interp, cnPtr->command, " -request ", 
				 ...
				 * about the window. */
{
    ...
    result = Tcl_VarEval(cnPtr->interp, cnPtr->command, " -lostslave ", 
	Tk_PathName(cnPtr->tkwin), (char*)NULL);
    ...
    cnPtr->isDeleted = 1;
    Tk_EventuallyFree((ClientData) cnPtr, FreeClientStruct);
}
...
TIX_DEFINE_CMD(Tix_ManageGeometryCmd)
{
    ...
    if (!inited) {
	Tcl_InitHashTable(&clientTable, sizeof(Tk_Window)/sizeof(int));
	inited = 1;
    }
