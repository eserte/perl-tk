    ...
    ClientData clientData;	/* Arbitrary data to pass to proc. */
{
    ...
    filePtr->mask = mask;
    filePtr->readyMask = 0;
    if (!filePtr->proc && filePtr->clientData) {
	LangFreeCallback((LangCallback *) filePtr->clientData);
    }
    filePtr->proc = proc;
    if (proc != NULL) {
				 ...
				 * handle, such as TCL_FILE_EVENTS. */
{
	...
	if (mask != 0) {
		...
		LangDoCallback(NULL,(LangCallback *) filePtr->clientData,
                               0,1,"%d",mask);
	    }
	}
	break;
    ...
    char **argv;		/* Argument strings. */
{
	...
	/* Reverse order - check for after# first as comparing callbacks is suspect */
	afterPtr = GetAfterEvent(assocPtr, LangString(arg));
	if (afterPtr == NULL) {
