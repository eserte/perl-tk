...
void
FontDebug(FontFamily *familyPtr,char *s,int n)
{
 LangDebug("-%s-%s-*-%s %s %d\n",
	familyPtr->foundry,
	familyPtr->faceName,
	Tcl_GetEncodingName(familyPtr->encoding),s,n);
}
				 ...
				 * terminating character. */
{
    ...
    /* FIXME - subFontArray can be re-allocated so lastSubFontPtr can
       end up pointing at memory which has been reused
     */
				 ...
				 * screen font must be able to display. */
{
	...
	hateCharset = charset;
	if (1 || strcmp(charset,"ucs-2be") != 0) {
	    /* Not a Unicode font discard it */
	    ReleaseSubFont(display, &subFont);
	}
	else {
	    /* Unicode font that cannot do _this_ character - keep it anyway */
	    if (0) FontDebug(subFont.familyPtr,"retain",fontPtr->numSubFonts);
    ...
    LangDebug("For %s U+%04X use %s\n",faceName,ch,nameList[nameIdx]);
    if (nameList != nameListOrig) {
	ckfree((char *) nameList);
    }
    XFreeFontNames(nameListOrig);

    if (0) FontDebug(subFont.familyPtr,"add",fontPtr->numSubFonts);
    if (fontPtr->numSubFonts >= SUBFONT_SPACE) {
	SubFont *newPtr;
    ...
    FontAttributes *gotPtr;	/* The attributes we have to live with. */
{
	...
	if (strcmp(gotPtr->xa.charset,"ucs-2be") != 0) {
	    /* Maybe reduce penalty for Unicode ? */
	}
	if (strcmp(gotAlias, wantAlias) != 0) {
        ...
        if (0) LangDebug("wanted %s got %s penalty = %u\n",wantAlias, gotAlias,penalty);
    }
    return penalty;
}
