...
extern void		TkComputeTextGeometry _ANSI_ARGS_((
			    ...
			    int *heightPtr));
MOVEXT int		TkCopyAndGlobalEval _ANSI_ARGS_((Tcl_Interp *interp,
			    char *script));
...
MOVEXT void		TkFillPolygon _ANSI_ARGS_((Tk_Canvas canvas,
...
MOVEXT int		TkGetMiterPoints _ANSI_ARGS_((double p1[], double p2[],
			    ...
			    double m2[]));
MOVEXT void		TkGetPointerCoords _ANSI_ARGS_((Tk_Window tkwin,
			    int *xPtr, int *yPtr));
COREXT void		TkGetServerInfo _ANSI_ARGS_((Tcl_Interp *interp,
			    Tk_Window tkwin));
