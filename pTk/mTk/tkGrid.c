...
static char sccsid[] = "@(#) tkGrid.c 1.16 96/01/30 18:26:42";
    ...
    char **argv;		/* Argument strings. */
{
	...
	if (row < 0 || column < 0) {
	    Tcl_AppendResult(interp, "grid position must be >= 0",(char *) NULL);
	    return TCL_ERROR;
	}
	...
	Tcl_IntResults(interp, 1, 1, slavePtr->gridColumn); 
	Tcl_AppendElement(interp, "-row");
	Tcl_IntResults(interp, 1, 1, slavePtr->gridRow); 
	Tcl_AppendElement(interp, "-columnspan");
	Tcl_IntResults(interp, 1, 1, slavePtr->gridWidth); 
	Tcl_AppendElement(interp, "-rowspan");
	Tcl_IntResults(interp, 1, 1, slavePtr->gridHeight); 
...
/*
	Tcl_AppendElement(interp, "-weightx");
	Tcl_DoubleResults(interp, 1, 1, slavePtr->weightX); 
	Tcl_AppendElement(interp, "-weighty");
	Tcl_DoubleResults(interp, 1, 1, slavePtr->weightY); 
*/
	...
	for (i=4; i<argc; i+=2) {
		...
		} else if (Tcl_GetInt(interp, argv[i+1], &size) != TCL_OK) {
		    return TCL_ERROR;
		} else {
    ...
    LayoutInfo *r;
{
	...
	for (slavePtr = masterPtr->slavePtr; slavePtr != NULL;
		...
		if (weight_diff > 0.0) {
		    ...
		    for (k = slavePtr->tempX; k < px; k++)
			weight += r->weightX[k];
		    for (k = slavePtr->tempX; weight > 0.0; k++) {
		...
		if (weight_diff > 0.0) {
		    ...
		    for (k = slavePtr->tempY; k < py; k++)
			weight += r->weightY[k];
		    for (k = slavePtr->tempY; weight > 0.0; k++) {
    ...
    XEvent *eventPtr;			/* Describes what just happened. */
{
	...
	gridPtr->tkwin = NULL;
	Tk_EventuallyFree((ClientData) gridPtr, (Tk_FreeProc *) DestroyGridBag);
    }
...
*/
	...
	struct GridBag *otherPtr;
    	char *lastWindow = NULL;	/* use this window to base current row/col on */
