...
static int		ConfigureMessage _ANSI_ARGS_((Tcl_Interp *interp,
			    Message *msgPtr, int argc, Arg *args,
			    int flags));
...
int
Tk_MessageCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    msgPtr->widgetCmd = Lang_CreateWidget(interp,msgPtr->tkwin, MessageWidgetCmd, (ClientData) msgPtr, MessageCmdDeletedProc);
    ...
    if (ConfigureMessage(interp, msgPtr, argc-2, args+2, 0) != TCL_OK) {
	goto error;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,msgPtr->tkwin));
    return TCL_OK;
...
static int
MessageWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	return Tk_ConfigureValue(interp, msgPtr->tkwin, configSpecs,
		(char *) msgPtr, LangString(args[2]), 0);
	...
	if (argc == 2) {
	    ...
	    return ConfigureMessage(interp, msgPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    } else {
...
static int
ConfigureMessage(interp, msgPtr, argc, args, flags)
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, msgPtr->tkwin, configSpecs,
	    argc, args, (char *) msgPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
