...
static int		ConfigureCanvas _ANSI_ARGS_((Tcl_Interp *interp,
			    TkCanvas *canvasPtr, int argc, Arg *args,
			    int flags));
...
static int		FindArea _ANSI_ARGS_((Tcl_Interp *interp,
			    TkCanvas *canvasPtr, Arg *args, Tk_Uid uid,
			    int enclosed));
...
int
Tk_CanvasCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    canvasPtr->widgetCmd = Lang_CreateWidget(interp,canvasPtr->tkwin, CanvasWidgetCmd, (ClientData) canvasPtr, CanvasCmdDeletedProc);
    ...
    if (ConfigureCanvas(interp, canvasPtr, argc-2, args+2, 0) != TCL_OK) {
	goto error;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,canvasPtr->tkwin));
    return TCL_OK;
    ...
    Arg *args;			/* Argument strings. */
{
    ...
    if ((c == 'a') && (strncmp(LangString(args[1]), "addtag", length) == 0)) {
	if (argc < 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " addtags tag searchCommand ?arg arg ...?\"",
		             NULL);
	    goto error;
	}
	result = FindItems(interp, canvasPtr, argc-3, args+3, LangString(args[2]), LangString(args[0]),
		" addtag tag");
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " bbox tagOrId ?tagOrId ...?\"",
		             NULL);
	    goto error;
	}
	gotAny = 0;
	for (i = 2; i < argc; i++) {
	    for (itemPtr = StartTagSearch(canvasPtr, LangString(args[i]), &search);
	...
	if ((argc < 3) || (argc > 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " bind tagOrId ?sequence? ?command?\"",
		             NULL);
	    goto error;
	}
	...
	object = 0;
	if (isdigit(UCHAR(LangString(args[2])[0]))) {
	    int id;
	    char *end;
	    ...
	    if (object == 0) {
		Tcl_AppendResult(interp, "item \"", LangString(args[2]),
			"\" doesn't exist",          NULL);
		goto error;
	    }
	} else {
	    bindByTag:
	    object = (ClientData) Tk_GetUid(LangString(args[2]));
	}
	...
	if (argc == 5) {
	    ...
	    mask = Tk_CreateBinding(interp, canvasPtr->bindingTable,
		    object, LangString(args[3]), args[4], append);
	    if (mask == 0) {
		goto error;
	    }
    ...
    } else if ((c == 'c') && (strcmp(LangString(args[1]), "canvasx") == 0)) {
	int x;
	double grid;

	if ((argc < 3) || (argc > 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " canvasx screenx ?gridspacing?\"",
		             NULL);
	    goto error;
	}
	if (Tk_GetPixels(interp, canvasPtr->tkwin, LangString(args[2]), &x) != TCL_OK) {
	    goto error;
	}
	if (argc == 4) {
	    if (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[3]),
		    &grid) != TCL_OK) {
		goto error;
	    }
	} else {
    ...
    } else if ((c == 'c') && (strcmp(LangString(args[1]), "canvasy") == 0)) {
	int y;
	double grid;

	if ((argc < 3) || (argc > 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " canvasy screeny ?gridspacing?\"",
		             NULL);
	    goto error;
	}
	if (Tk_GetPixels(interp, canvasPtr->tkwin, LangString(args[2]), &y) != TCL_OK) {
	    goto error;
	}
	if (argc == 4) {
	    if (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr,
		    LangString(args[3]), &grid) != TCL_OK) {
		goto error;
	    }
	} else {
	    grid = 0.0;
	...
	result = Tk_ConfigureValue(interp, canvasPtr->tkwin, configSpecs,
		(char *) canvasPtr, LangString(args[2]), 0);
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " coords tagOrId ?x y x y ...?\"",
		             NULL);
	    goto error;
	}
	itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	if (itemPtr != NULL) {
	    if (argc != 3) {
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " create type ?arg arg ...?\"",          NULL);
	    goto error;
	}
	...
	for (typePtr = typeList; typePtr != NULL; typePtr = typePtr->nextPtr) {
	    if ((c == typePtr->name[0])
		    && (strncmp(LangString(args[2]), typePtr->name, length) == 0)) {
		if (matchPtr != NULL) {
		    badType:
	...
	Tcl_IntResults(interp,1,0, itemPtr->id);
	...
	if ((argc != 4) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " dchars tagOrId first ?last?\"",
		             NULL);
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
		itemPtr != NULL; itemPtr = NextItem(&search)) {
	    ...
	    if ((*itemPtr->typePtr->indexProc)(interp, (Tk_Canvas) canvasPtr,
		    itemPtr, args[3], &first) != TCL_OK) {
		goto error;
	    }
	    if (argc == 5) {
		if ((*itemPtr->typePtr->indexProc)(interp,
			(Tk_Canvas) canvasPtr, itemPtr, args[4], &last)
			!= TCL_OK) {
		    goto error;
		}
	    } else {
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " dtag tagOrId ?tagToDelete?\"",
		             NULL);
	    goto error;
	}
	if (argc == 4) {
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " find searchCommand ?arg arg ...?\"",
		             NULL);
	    goto error;
	}
	result = FindItems(interp, canvasPtr, argc-2, args+2,          NULL,
		LangString(args[0])," find");
    } else if ((c == 'f') && (strncmp(LangString(args[1]), "focus", length) == 0)
	    && (length >= 2)) {
	if (argc > 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " focus ?tagOrId?\"",
		             NULL);
	    goto error;
	}
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " gettags tagOrId\"",          NULL);
	    goto error;
	}
	itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	if (itemPtr != NULL) {
    ...
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "icursor", length) == 0)
	    && (length >= 2)) {
	int index;

	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " icursor tagOrId index\"",
		             NULL);
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " index tagOrId string\"",
		             NULL);
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	...
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " insert tagOrId beforeThis string\"",
		             NULL);
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	    ...
	    if ((*itemPtr->typePtr->indexProc)(interp, (Tk_Canvas) canvasPtr,
		    itemPtr, args[3], &beforeThis) != TCL_OK) {
		goto error;
	    }
	    ...
	    (*itemPtr->typePtr->insertProc)((Tk_Canvas) canvasPtr,
		    itemPtr, beforeThis, LangString(args[4]));
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " itemccget tagOrId option\"",
		             NULL);
	    return TCL_ERROR;
	}
	itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " itemconfigure tagOrId ?option value ...?\"",
		             NULL);
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " lower tagOrId ?belowThis?\"",
		             NULL);
	    goto error;
	}
	...
	if (argc == 3) {
	    ...
	    prevPtr = StartTagSearch(canvasPtr, LangString(args[3]), &search);
	    if (prevPtr != NULL) {
		prevPtr = search.prevPtr;
	    } else {
		Tcl_AppendResult(interp, "tag \"", LangString(args[3]),
			"\" doesn't match any items",          NULL);
		goto error;
	    }
	}
	RelinkItems(canvasPtr, LangString(args[2]), prevPtr);
    } else if ((c == 'm') && (strncmp(LangString(args[1]), "move", length) == 0)) {
	double xAmount, yAmount;

	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " move tagOrId xAmount yAmount\"",
		             NULL);
	    goto error;
	}
	if ((Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[3]),
		&xAmount) != TCL_OK) || (Tk_CanvasGetCoord(interp,
		(Tk_Canvas) canvasPtr, LangString(args[4]), &yAmount) != TCL_OK)) {
	    goto error;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
		itemPtr != NULL; itemPtr = NextItem(&search)) {
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " raise tagOrId ?aboveThis?\"",
		             NULL);
	    goto error;
	}
	...
	if (argc == 3) {
	    ...
	    prevPtr = NULL;
	    for (itemPtr = StartTagSearch(canvasPtr, LangString(args[3]), &search);
		    itemPtr != NULL; itemPtr = NextItem(&search)) {
		prevPtr = itemPtr;
	    }
	    if (prevPtr == NULL) {
		Tcl_AppendResult(interp, "tagOrId \"", LangString(args[3]),
			"\" doesn't match any items",          NULL);
		goto error;
	    }
	}
	...
	if (argc != 7) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " scale tagOrId xOrigin yOrigin xScale yScale\"",
		             NULL);
	    goto error;
	}
	if ((Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr,
		    LangString(args[3]), &xOrigin) != TCL_OK)
		|| (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr,
		    LangString(args[4]), &yOrigin) != TCL_OK)
		|| (Tcl_GetDouble(interp, args[5], &xScale) != TCL_OK)
		|| (Tcl_GetDouble(interp, args[6], &yScale) != TCL_OK)) {
	    goto error;
	}
    ...
    } else if ((c == 's') && (strncmp(LangString(args[1]), "scan", length) == 0)
	    && (length >= 3)) {
	int x, y;
	...
	if ((LangString(args[2])[0] == 'm')
		&& (strncmp(LangString(args[2]), "mark", strlen(LangString(args[2]))) == 0)) {
	    canvasPtr->scanX = x;
	    canvasPtr->scanXOrigin = canvasPtr->xOrigin;
	    canvasPtr->scanY = y;
	    canvasPtr->scanYOrigin = canvasPtr->yOrigin;
	} else if ((LangString(args[2])[0] == 'd')
		&& (strncmp(LangString(args[2]), "dragto", strlen(LangString(args[2]))) == 0)) {
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " select option ?tagOrId? ?arg?\"",          NULL);
	    goto error;
	}
	if (argc >= 4) {
	    for (itemPtr = StartTagSearch(canvasPtr, LangString(args[3]), &search);
	...
	if ((c == 'a') && (strncmp(LangString(args[2]), "adjust", length) == 0)) {
	    if (argc != 5) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " select adjust tagOrId index\"",
			         NULL);
		goto error;
	    }
	...
	} else if ((c == 'c') && (LangString(args[2]) != NULL)
		&& (strncmp(LangString(args[2]), "clear", length) == 0)) {
	    if (argc != 3) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " select clear\"",          NULL);
		goto error;
	    }
	    ...
	    if (argc != 5) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " select from tagOrId index\"",
			         NULL);
		goto error;
	    }
	    ...
	    if (argc != 3) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " select item\"",          NULL);
		goto error;
	    }
	    if (canvasPtr->textInfo.selItemPtr != NULL) {
		Tcl_IntResults(interp,1,0,
			canvasPtr->textInfo.selItemPtr->id);
	    }
	} else if ((c == 't') && (strncmp(LangString(args[2]), "to", length) == 0)) {
	    if (argc != 5) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " select to tagOrId index\"",
			         NULL);
		goto error;
	    }
	    CanvasSelectTo(canvasPtr, itemPtr, index);
	} else {
	    Tcl_AppendResult(interp, "bad select option \"", LangString(args[2]),
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " type tag\"",          NULL);
	    goto error;
	}
	itemPtr = StartTagSearch(canvasPtr, LangString(args[2]), &search);
	if (itemPtr != NULL) {
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, canvasPtr->tkwin, configSpecs,
	    argc, args, (char *) canvasPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
	...
	if (Lang_SplitList(canvasPtr->interp, canvasPtr->regionArg,
		&argc2, &args2, &freeProc) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (argc2 != 4) {
	    badRegion:
 ...
 *	to Tcl_GetResult(interp);  otherwise tag is added to itemPtr's
 *	list of tags.
 *
 *--------------------------------------------------------------
 */
 ...
 *	list of ids from all the items that match argc/args is
 *	returned in Tcl_GetResult(interp).  If newTag is NULL, then
 *	the normal Tcl_GetResult(interp) is an empty string.  If an error
 *	occurs, then Tcl_GetResult(interp) will hold an error message.
 *
 * Side effects:
 *	If newTag is non-NULL, then all the items that match the
 *	information in argc/args have that tag added to their
 *	lists of tags.
 *
 *--------------------------------------------------------------
 */
...
static int
FindItems(interp, canvasPtr, argc, args, newTag, cmdName, option)
    Tcl_Interp *interp;			/* Interpreter for error reporting. */
    TkCanvas *canvasPtr;		/* Canvas whose items are to be
					 * searched. */
    int argc;				/* Number of entries in args.  Must be
					 * greater than zero. */
    Arg *args;			/* Arguments that describe what items
					 ...
					 * up to what's in argc/args. */
{
	...
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args:  must be \"",
		    cmdName, option, " above tagOrId",          NULL);
	    return TCL_ERROR;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[1]), &search);
		itemPtr != NULL; itemPtr = NextItem(&search)) {
	    lastPtr = itemPtr;
	}
	...
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args:  must be \"",
		    cmdName, option, " below tagOrId",          NULL);
	    return TCL_ERROR;
	}
	itemPtr = StartTagSearch(canvasPtr, LangString(args[1]), &search);
	if (search.prevPtr != NULL) {
	    DoItem(interp, search.prevPtr, uid);
	}
    } else if ((c == 'c') && (strncmp(LangString(args[0]), "closest", length) == 0)) {
	double closestDist;
	...
	if ((Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[1]),
		&coords[0]) != TCL_OK) || (Tk_CanvasGetCoord(interp,
		(Tk_Canvas) canvasPtr, LangString(args[2]), &coords[1]) != TCL_OK)) {
	    return TCL_ERROR;
	}
	if (argc > 3) {
	    if (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[3]),
	...
	if (argc == 5) {
	    itemPtr = StartTagSearch(canvasPtr, LangString(args[4]), &search);
	    if (itemPtr != NULL) {
		startPtr = itemPtr;
	    }
	}
	...
	if (argc != 5) {
	    Tcl_AppendResult(interp, "wrong # args:  must be \"",
		    cmdName, option, " enclosed x1 y1 x2 y2",          NULL);
	    return TCL_ERROR;
	}
	return FindArea(interp, canvasPtr, args+1, uid, 1);
	...
	return FindArea(interp, canvasPtr, args+1, uid, 0);
    } else if ((c == 'w') && (strncmp(LangString(args[0]), "withtag", length) == 0)) {
		if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args:  must be \"",
		    cmdName, option, " withtag tagOrId",          NULL);
	    return TCL_ERROR;
	}
	for (itemPtr = StartTagSearch(canvasPtr, LangString(args[1]), &search);
		itemPtr != NULL; itemPtr = NextItem(&search)) {
 ...
 *	by the rectangle given by argc is returned in Tcl_GetResult(interp).
 *	If newTag is NULL, then the normal Tcl_GetResult(interp) is an
 *	empty string.  If an error occurs, then Tcl_GetResult(interp) will
 *	hold an error message.
 *
 * Side effects:
 *	If uid is non-NULL, then all the items overlapping
 *	or enclosed by the area in args have that tag added to
 *	their lists of tags.
 *
 *--------------------------------------------------------------
 */
...
FindArea(interp, canvasPtr, args, uid, enclosed)
    ...
    TkCanvas *canvasPtr;		/* Canvas whose items are to be
					 * searched. */
    Arg *args;			/* Array of four arguments that
					 ...
					 * items are OK. */
{
    ...
    if ((Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[0]),
		&rect[0]) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[1]),
		&rect[1]) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[2]),
		&rect[2]) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, (Tk_Canvas) canvasPtr, LangString(args[3]),
		&rect[3]) != TCL_OK)) {
	return TCL_ERROR;
    }
    if (rect[0] > rect[2]) {
    ...
    TkCanvas *canvasPtr;		/* Information about canvas. */
{
	...
	PrintScrollFractions(canvasPtr->xOrigin + canvasPtr->inset,
		...
		&first, &last);
	result = LangDoCallback(canvasPtr->interp, canvasPtr->xScrollCmd, 0, 2, " %g %g", first, last);
	if (result != TCL_OK) {
