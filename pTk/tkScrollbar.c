...
static int		ConfigureScrollbar _ANSI_ARGS_((Tcl_Interp *interp,
			    Scrollbar *scrollPtr, int argc, Arg *args,
			    int flags));
...
int
Tk_ScrollbarCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    if (ConfigureScrollbar(interp, scrollPtr, argc-2, args+2, 0) != TCL_OK) {
	goto error;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,scrollPtr->tkwin));
    return TCL_OK;
...
static int
ScrollbarWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;			/* Argument strings. */
{
	...
	if (argc == 2) {
		...
		case TOP_ARROW:		Tcl_SetResult(interp, "arrow1",TCL_STATIC);	break;
		case SLIDER:		Tcl_SetResult(interp, "slider",TCL_STATIC);	break;
		case BOTTOM_ARROW:	Tcl_SetResult(interp, "arrow2",TCL_STATIC);	break;
	    }
	    goto done;
	}
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " activate element\"",          NULL);
	    goto error;
	}
	c = LangString(args[2])[0];
	length = strlen(LangString(args[2]));
	if ((c == 'a') && (strcmp(LangString(args[2]), "arrow1") == 0)) {
	    scrollPtr->activeField = TOP_ARROW;
	} else if ((c == 'a') && (strcmp(LangString(args[2]), "arrow2") == 0)) {
	    scrollPtr->activeField = BOTTOM_ARROW;
	} else if ((c == 's') && (strncmp(LangString(args[2]), "slider", length) == 0)) {
	...
	result = Tk_ConfigureValue(interp, scrollPtr->tkwin, configSpecs,
		(char *) scrollPtr, LangString(args[2]), 0);
    } else if ((c == 'c') && (strncmp(LangString(args[1]), "configure", length) == 0)
	    && (length >= 2)) {
	if (argc == 2) {
	    result = Tk_ConfigureInfo(interp, scrollPtr->tkwin, configSpecs,
		    (char *) scrollPtr,          NULL, 0);
	} else if (argc == 3) {
	    result = Tk_ConfigureInfo(interp, scrollPtr->tkwin, configSpecs,
		    (char *) scrollPtr, LangString(args[2]), 0);
	} else {
	    result = ConfigureScrollbar(interp, scrollPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    } else if ((c == 'd') && (strncmp(LangString(args[1]), "delta", length) == 0)) {
	int xDelta, yDelta, pixels, length;
	double fraction;

	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " delta xDelta yDelta\"",          NULL);
	    goto error;
	}
	if ((Tcl_GetInt(interp, args[2], &xDelta) != TCL_OK)
		|| (Tcl_GetInt(interp, args[3], &yDelta) != TCL_OK)) {
	    goto error;
	}
	...
	Tcl_DoubleResults(interp,1,0, fraction);
    } else if ((c == 'f') && (strncmp(LangString(args[1]), "fraction", length) == 0)) {
	int x, y, pos, length;
	double fraction;

	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " fraction x y\"",          NULL);
	    goto error;
	}
	...
	if (scrollPtr->flags & NEW_STYLE_COMMANDS) {
	    ...
	    Tcl_IntResults(interp,4,0, scrollPtr->totalUnits,
	...
	switch (thing) {
	    case TOP_ARROW:	Tcl_SetResult(interp, "arrow1",TCL_STATIC);	break;
	    case TOP_GAP:	Tcl_SetResult(interp, "trough1",TCL_STATIC);	break;
	    case SLIDER:	Tcl_SetResult(interp, "slider",TCL_STATIC);	break;
	    case BOTTOM_GAP:	Tcl_SetResult(interp, "trough2",TCL_STATIC);	break;
	    case BOTTOM_ARROW:	Tcl_SetResult(interp, "arrow2",TCL_STATIC);	break;
	}
	...
	if (argc == 4) {
	    ...
	    if (Tcl_GetDouble(interp, args[2], &first) != TCL_OK) {
		goto error;
	    }
	    if (Tcl_GetDouble(interp, args[3], &last) != TCL_OK) {
		goto error;
	    }
	    if (first < 0) {
	    ...
	    if (Tcl_GetInt(interp, args[2], &totalUnits) != TCL_OK) {
		goto error;
	    }
	    if (totalUnits < 0) {
		totalUnits = 0;
	    }
	    if (Tcl_GetInt(interp, args[3], &windowUnits) != TCL_OK) {
		goto error;
	    }
	    if (windowUnits < 0) {
		windowUnits = 0;
	    }
	    if (Tcl_GetInt(interp, args[4], &firstUnit) != TCL_OK) {
		goto error;
	    }
	    if (Tcl_GetInt(interp, args[5], &lastUnit) != TCL_OK) {
		goto error;
	    }
	    if (totalUnits > 0) {
	    ...
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " set firstFraction lastFraction\" or \"",
		    LangString(args[0]),
...
static int
ConfigureScrollbar(interp, scrollPtr, argc, args, flags)
					 ...
					 * Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, scrollPtr->tkwin, configSpecs,
	    argc, args, (char *) scrollPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
