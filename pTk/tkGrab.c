 ...
 */
	...
	/* ARGSUSED */
int
Tk_GrabCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " ?-global? window\" or \"", LangString(args[0]),
		" option ?arg arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    ...
    } else if ((c == 'c') && (strncmp(LangString(args[1]), "current", length) == 0)) {
	if (argc > 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " current ?window?\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    tkwin = Tk_NameToWindow(interp, LangString(args[2]), (Tk_Window) clientData);
		...
		Tcl_ArgResult(interp,LangWidgetArg(interp,(Tk_Window)(dispPtr->eventualGrabWinPtr)));
	    }
	} else {
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " release window\"",          NULL);
	    return TCL_ERROR;
	}
	tkwin = Tk_NameToWindow(interp, LangString(args[2]), (Tk_Window) clientData);
	if (tkwin == NULL) {
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " set ?-global? window\"",          NULL);
	    return TCL_ERROR;
	}
	if (argc == 3) {
	    globalGrab = 0;
	    ...
	    tkwin = Tk_NameToWindow(interp, LangString(args[3]), (Tk_Window) clientData);
	}
	if (tkwin == NULL) {
	    return TCL_ERROR;
	}
	return Tk_Grab(interp, tkwin, globalGrab);
    } else if ((c == 's') && (strncmp(LangString(args[1]), "status", length) == 0)
	    && (length >= 2)) {
	TkWindow *winPtr;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " status window\"",          NULL);
	    return TCL_ERROR;
	}
	winPtr = (TkWindow *) Tk_NameToWindow(interp, LangString(args[2]),
 ...
 *	and Tcl_GetResult(interp) will hold an error message.
 *
 * Side effects:
