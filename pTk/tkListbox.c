...
static int		ConfigureListbox _ANSI_ARGS_((Tcl_Interp *interp,
			    Listbox *listPtr, int argc, Arg *args,
			    int flags));
...
int
Tk_ListboxCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    listPtr->widgetCmd = Lang_CreateWidget(interp,listPtr->tkwin, ListboxWidgetCmd, (ClientData) listPtr, ListboxCmdDeletedProc);
    ...
    if (ConfigureListbox(interp, listPtr, argc-2, args+2, 0) != TCL_OK) {
	goto error;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,listPtr->tkwin));
    return TCL_OK;
...
static int
ListboxWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;			/* Argument strings. */
{
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " activate index\"",
		             NULL);
	    goto error;
	}
	...
	if ((index >= listPtr->topIndex) && (index < listPtr->numElements)
	    ...
	    Tcl_IntResults(interp,4,0, x, y, elPtr->pixelWidth,
	...
	result = Tk_ConfigureValue(interp, listPtr->tkwin, configSpecs,
		(char *) listPtr, LangString(args[2]), 0);
	...
	if (argc == 2) {
	    ...
	    result = ConfigureListbox(interp, listPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    } else if ((c == 'c') && (strncmp(LangString(args[1]), "curselection", length) == 0)
	    && (length >= 2)) {
	int i, count;
	char index[20];
	Element *elPtr;
	...
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " curselection\"",
		             NULL);
	    goto error;
	}
	count = 0;
	...
	if ((argc < 3) || (argc > 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " delete firstIndex ?lastIndex?\"",
		             NULL);
	    goto error;
	}
	...
	if ((argc != 3) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " get first ?last?\"",          NULL);
	    goto error;
	}
	if (GetListboxIndex(interp, listPtr, args[2], 0, &first) != TCL_OK) {
	    goto error;
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " insert index ?element element ...?\"",
		             NULL);
	    goto error;
	}
	if (GetListboxIndex(interp, listPtr, args[2], 1, &index)
	...
	InsertEls(listPtr, index, argc-3, args+3);
    } else if ((c == 'n') && (strncmp(LangString(args[1]), "nearest", length) == 0)) {
	int index, y;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " nearest y\"",          NULL);
	    goto error;
	}
	...
	Tcl_IntResults(interp,1,0, index);
	    ...
	    && (strncmp(LangString(args[1]), "scan", length) == 0)) {
	int x, y;
    ...
    } else if ((c == 's') && (strncmp(LangString(args[1]), "see", length) == 0)
	    && (length >= 3)) {
	int index, diff;
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " see index\"",
		             NULL);
	    goto error;
	}
	if (GetListboxIndex(interp, listPtr, args[2], 0, &index) != TCL_OK) {
	...
	if (diff > 0) {
	    ...
	    && (strncmp(LangString(args[1]), "selection", length) == 0)) {
	int first, last;

	if ((argc != 4) && (argc != 5)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " selection option index ?index?\"",
		             NULL);
	    goto error;
	}
	...
	if ((c == 'a') && (strncmp(LangString(args[2]), "anchor", length) == 0)) {
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " selection anchor index\"",          NULL);
		goto error;
	    }
	    listPtr->selectAnchor = first;
	} else if ((c == 'c') && (strncmp(LangString(args[2]), "clear", length) == 0)) {
	    ListboxSelect(listPtr, first, last, 0);
	} else if ((c == 'i') && (strncmp(LangString(args[2]), "includes", length) == 0)) {
	    int i;
	    Element *elPtr;
    
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection includes index\"",          NULL);
		goto error;
	    }
	    for (elPtr = listPtr->firstPtr, i = 0; i < first;
		    i++, elPtr = elPtr->nextPtr) {
	    ...
	    Tcl_AppendResult(interp, "bad selection option \"", LangString(args[2]),
	...
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " size\"",          NULL);
	    goto error;
	}
	Tcl_IntResults(interp,1,0, listPtr->numElements);
...
static int
ConfigureListbox(interp, listPtr, argc, args, flags)
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, listPtr->tkwin, configSpecs,
	    argc, args, (char *) listPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 *	string.  Otherwise an error message is left in Tcl_GetResult(interp).
 *
 * Side effects:
 *	None.
 *
    ...
    register Listbox *listPtr;		/* Information about widget. */
{
    ...
    result = LangDoCallback(listPtr->interp, listPtr->yScrollCmd, 0, 2, " %g %g", first, last);
    if (result != TCL_OK) {
    ...
    register Listbox *listPtr;		/* Information about widget. */
{
    ...
    result = LangDoCallback(listPtr->interp, listPtr->xScrollCmd, 0, 2, " %g %g",first, last);
    if (result != TCL_OK) {
	Tcl_AddErrorInfo(listPtr->interp,
