...
int
Tk_OptionCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tk_AddOption(tkwin, LangString(args[2]), LangString(args[3]), priority);
	return TCL_OK;
	...
	value = Tk_GetOption(window, LangString(args[3]), LangString(args[4]));
	if (value != NULL) {
 ...
 *	error message will be left in Tcl_GetResult(interp).  The memory at
				 ...
				 * 0 and TK_MAX_PRIO. */
{
	...
	while (*src != ':') {
	    if ((*src == '\0') || (*src == '\n')) {
		Tcl_SprintfResult(interp, "missing colon on line %d",
			lineNum);
		return TCL_ERROR;
	    }
	    if ((src[0] == '\\') && (src[1] == '\n')) {
	...
	*dst = '\0';
	...
	if (*src == '\0') {
	    Tcl_SprintfResult(interp, "missing value on line %d", lineNum);
	    return TCL_ERROR;
	}
	...
	while (*src != '\n') {
	    if (*src == '\0') {
		Tcl_SprintfResult(interp, "missing newline on line %d",
			lineNum);
		return TCL_ERROR;
	    }
	    if ((src[0] == '\\') && (src[1] == '\n')) {
