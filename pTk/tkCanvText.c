...
static int
CreateText(interp, canvas, itemPtr, argc, args)
    ...
    Arg *args;			/* Arguments describing rectangle. */
{
	...
	Tcl_AppendResult(interp, "wrong # args:  should be \"",
		...
		itemPtr->typePtr->name, " x y [options]",          NULL);
	return TCL_ERROR;
    }
    ...
    if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &textPtr->x) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]), &textPtr->y)
		!= TCL_OK)) {
	return TCL_ERROR;
    }

    if (ConfigureText(interp, canvas, itemPtr, argc-2, args+2, 0) != TCL_OK) {
...
static int
TextCoords(interp, canvas, itemPtr, argc, args)
...
static int
ConfigureText(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
 ...
 *	Tcl_GetResult(interp).  It generates proper backslash notation so
