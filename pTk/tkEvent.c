...
typedef struct BgError {
    ...
    char *errorMsg;		/* The error message (Tcl_GetResult(interp) when
				 ...
				 * occurred. */
{
    ...
    errPtr->errorMsg = (char *) ckalloc((unsigned) (strlen(Tcl_GetResult(interp))
	    + 1));
    strcpy(errPtr->errorMsg, Tcl_GetResult(interp));
    ...
    ClientData clientData;		/* Not used. */
{
	...
	LangSetString(args+0, "tkerror");
	LangSetString(args+1, firstBgPtr->errorMsg);
	command = Tcl_Merge(2, args);
	Tcl_AllowExceptions(interp);
	code = Tcl_GlobalEval(interp, command);
	ckfree(command);
	if (code == TCL_ERROR) {
	    if (strcmp(Tcl_GetResult(interp), "\"tkerror\" is an invalid command name or ambiguous abbreviation") == 0) {
 ...
 */
	...
	/* ARGSUSED */
int
Tk_AfterCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " milliseconds ?command? ?arg arg ...?\" or \"",
		LangString(args[0]), " cancel id|command\"",          NULL);
	return TCL_ERROR;
    }

    if (isdigit(UCHAR(LangString(args[1])[0]))) {
	if (Tcl_GetInt(interp, args[1], &ms) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (ms < 0) {
	    ms = 0;
	...
	if (argc == 3) {
	    ...
	    afterPtr->command = LangMakeCallback(Tcl_Merge(argc-2, args+2));
	}
	afterPtr->id = nextId;
	nextId += 1;
	...
	Tcl_SprintfResult(interp, "after#%d", afterPtr->id);
    } else if (strncmp(LangString(args[1]), "cancel", strlen(LangString(args[1]))) == 0) {
	Arg arg;
    ...
    } else if (strncmp(LangString(args[1]), "idle", strlen(LangString(args[1]))) == 0) {
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " idle script script ...\"",          NULL);
	    return TCL_ERROR;
	}
	afterPtr = (AfterInfo *) ckalloc((unsigned) (sizeof(AfterInfo)));
	afterPtr->interp = interp;
	...
	Tcl_AppendResult(interp, "bad argument \"", LangString(args[1]),
		"\": must be cancel, idle, or a number",          NULL);
	return TCL_ERROR;
    }
    return TCL_OK;
}
 ...
 */
	...
	/* ARGSUSED */
int
Tk_FileeventCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    if ((argc != 3) && (argc != 4)) {
	Tcl_AppendResult(interp, "wrong # args: must be \"", LangString(args[0]),
		" fileId event ?script?",          NULL);
	return TCL_ERROR;
    }
    c = LangString(args[2])[0];
    length = strlen(LangString(args[2]));
    if ((c == 'r') && (strncmp(LangString(args[2]), "readable", length) == 0)) {
	index = 0;
    } else if ((c == 'w') && (strncmp(LangString(args[2]), "writable", length) == 0)) {
	index = 1;
    } else {
	Tcl_AppendResult(interp, "bad event name \"", LangString(args[2]),
		"\": must be readable or writable",          NULL);
	return TCL_ERROR;
    }
	...
	if (fevPtr == NULL) {
	    if ((argc == 3) || (LangString(args[3])[0] == 0)) {
		return TCL_OK;
	    }
 ...
 */
	...
	/* ARGSUSED */
static int
TkwaitCmd2(clientData, interp, argc, args)
    ClientData clientData;	/* Not used. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (argc != 3) {
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " variable name\"",          NULL);
	return TCL_ERROR;
    }
 ...
 */
	...
	/* ARGSUSED */
static int
UpdateCmd2(clientData, interp, argc, args)
    ClientData clientData;	/* Not used. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
...
Tk_EventInit(interp)
    Tcl_Interp *interp;		/* Interpreter in which to set up
				 * event-handling. */
{
    Tcl_CreateCommand(interp, "after", Tk_AfterCmd, (ClientData) NULL, (void (*)()) NULL);

    Tcl_CreateCommand(interp, "fileevent", Tk_FileeventCmd, (ClientData) NULL, (void (*)()) NULL);

    Tcl_CreateCommand(interp, "tkwait", TkwaitCmd2, (ClientData) NULL, (void (*)()) NULL);

    Tcl_CreateCommand(interp, "update", UpdateCmd2, (ClientData) NULL, (void (*)()) NULL);
