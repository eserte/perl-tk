...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_COLOR, "-outline",          NULL,          NULL,
	         NULL, Tk_Offset(PolygonItem, outlineColor), TK_CONFIG_NULL_OK},
...
static int
CreatePolygon(interp, canvas, itemPtr, argc, args)
    Tcl_Interp *interp;			/* Interpreter for error reporting. */
    Tk_Canvas canvas;			/* Canvas to hold new item. */
    Tk_Item *itemPtr;			/* Record to hold new item;  header
					 * has been initialized by caller. */
    int argc;				/* Number of arguments in args. */
    Arg *args;			/* Arguments describing polygon. */
{
	...
	Tcl_AppendResult(interp, "wrong # args:  should be \"",
		...
		" x1 y1 x2 y2 x3 y3 ?x4 y4 ...? ?options?",          NULL);
	return TCL_ERROR;
    }
	...
	if ((!isdigit(UCHAR(LangString(args[i])[0]))) &&
		((LangString(args[i])[0] != '-') || (!isdigit(UCHAR(LangString(args[i])[1]))))) {
	    break;
	}
    }
    if (PolygonCoords(interp, canvas, itemPtr, i, args) != TCL_OK) {
	goto error;
    }

    if (ConfigurePolygon(interp, canvas, itemPtr, argc-i, args+i, 0)
	    == TCL_OK) {
	return TCL_OK;
    }
...
static int
PolygonCoords(interp, canvas, itemPtr, argc, args)
...
static int
ConfigurePolygon(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
