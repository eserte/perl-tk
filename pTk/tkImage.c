...
int
Tk_ImageCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    if ((c == 'c') && (strncmp(LangString(args[1]), "create", length) == 0)) {
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"", LangString(args[0]),
		    " create type ?name? ?options?\"",          NULL);
	    return TCL_ERROR;
	}
	...
	for (typePtr = imageTypeList; typePtr != NULL;
	    ...
	    if ((c == typePtr->name[0])
		    && (strcmp(LangString(args[2]), typePtr->name) == 0)) {
		break;
	    }
	}
	if (typePtr == NULL) {
	    Tcl_AppendResult(interp, "image type \"", LangString(args[2]),
		    "\" doesn't exist",          NULL);
	    return TCL_ERROR;
	}
	...
	if ((*typePtr->createProc)(interp, name, argc-firstOption,
		args+firstOption, typePtr, (Tk_ImageMaster) masterPtr,
	...
	for (i = 2; i < argc; i++) {
	    ...
	    if (hPtr == NULL) {
	    Tcl_AppendResult(interp, "image \"", LangString(args[i]),
		    "\" doesn't exist",          NULL);
		return TCL_ERROR;
	    }
	    masterPtr = (ImageMaster *) Tcl_GetHashValue(hPtr);
	...
	hPtr = Tcl_FindHashEntry(&winPtr->mainPtr->imageTable, LangString(args[2]));
	if (hPtr == NULL) {
	    Tcl_AppendResult(interp, "image \"", LangString(args[2]),
		    "\" doesn't exist",          NULL);
	    return TCL_ERROR;
	}
	masterPtr = (ImageMaster *) Tcl_GetHashValue(hPtr);
	Tcl_IntResults(interp,1,0, masterPtr->height);
    } else if ((c == 'n') && (strncmp(LangString(args[1]), "names", length) == 0)) {
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"", LangString(args[0]),
		    " names\"",          NULL);
	    return TCL_ERROR;
    ...
    } else if ((c == 't') && (strcmp(LangString(args[1]), "types") == 0)) {
	if (argc != 2) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"", LangString(args[0]),
		    " types\"",          NULL);
	    return TCL_ERROR;
	...
	masterPtr = (ImageMaster *) Tcl_GetHashValue(hPtr);
	Tcl_IntResults(interp,1,0, masterPtr->width);
    } else {
