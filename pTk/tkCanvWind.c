...
static int
CreateWinItem(interp, canvas, itemPtr, argc, args)
    ...
    Arg *args;			/* Arguments describing rectangle. */
{
    ...
    if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &winItemPtr->x) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]),
		&winItemPtr->y) != TCL_OK)) {
	return TCL_ERROR;
    }

    if (ConfigureWinItem(interp, canvas, itemPtr, argc-2, args+2, 0)
	    != TCL_OK) {
...
static int
WinItemCoords(interp, canvas, itemPtr, argc, args)
					 ...
					 * x2, y2, ... */
{
    ...
    if (argc == 0) {
	Tcl_DoubleResults(interp, 2, 1, winItemPtr->x, winItemPtr->y);
    } else if (argc == 2) {
	if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &winItemPtr->x)
		!= TCL_OK) || (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]),
		&winItemPtr->y) != TCL_OK)) {
	    return TCL_ERROR;
	}
...
static int
ConfigureWinItem(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, canvasTkwin, configSpecs, argc, args,
