				 ...
				 * 0 means consider these invalid. */
{
	...
	result = 0;
#if 0
	if (Tcl_SplitList((Tcl_Interp *) NULL, property, &argc, &args)
		== TCL_OK) {
	    for (i = 0; i < argc; i++) {
		if (strcmp(LangString(args[i]), name) == 0) {
		    result = 1;
		    break;
		}
	    }
...
#else
        /* FIXME Approximation to above */
        if (!strncmp(property,name,strlen(name))) {
		result = 1;
	}
#endif
    } else {
				 ...
				 * unique. */
{
	...
	if (riPtr == NULL) {
	    ...
	    registry = riPtr;
	    Tcl_CreateCommand(interp, "send", Tk_SendCmd, (ClientData) riPtr, DeleteProc);
...
int
Tk_SendCmd(clientData, interp, argc, args)
    ...
    Arg *args;			/* Argument strings. */
{
	...
	if ((c == 'a') &&  LangCmpOpt("-async",LangString(args[i]),length) == 0 ) {
	    ...
	    i += 2;
	} else if (strcmp(LangString(args[i]), "--") == 0) {
	    i++;
	    break;
	} else {
	    Tcl_AppendResult(interp, "bad option \"", LangString(args[i]),
	...
	if (firstArg == (argc-1)) {
	    result = Tcl_GlobalEval(riPtr->interp, LangString(args[firstArg]));
	} else {
	    Tcl_DStringInit(&request);
	    Tcl_DStringAppend(&request, LangString(args[firstArg]), -1);
	    for (i = firstArg+1; i < argc; i++) {
		Tcl_DStringAppend(&request, " ", 1);
		Tcl_DStringAppend(&request, LangString(args[i]), -1);
	    }
	...
	if ((riPtr->interp != NULL) && interp != riPtr->interp) {
		...
		Tcl_AddErrorInfo(interp,  Lang_GetErrorInfo(riPtr->interp));

		Lang_SetErrorCode(interp, Lang_GetErrorCode(riPtr->interp));
	    }
	    Tcl_SetResult(interp, Tcl_GetResult(riPtr->interp), TCL_VOLATILE);
 ...
 *	is returned and Tcl_GetResult(interp) will hold an error message.
 *
 * Side effects:
 *	None.
 *
    ...
    XEvent *eventPtr;		/* Information about event. */
{
	...
	if ((*p == 'c') && (p[1] == 0)) {
		...
		Tcl_DStringAppend(&reply, Tcl_GetResult(riPtr->interp), -1);
		if (result == TCL_ERROR) {
		    char *varValue;
    
		    varValue = Lang_GetErrorInfo(riPtr->interp);
		    if (varValue != NULL) {
			Tcl_DStringAppend(&reply, "\0-i ", 4);
			Tcl_DStringAppend(&reply, varValue, -1);
		    }
		    varValue = Lang_GetErrorCode(riPtr->interp);
		    if (varValue != NULL) {
...
#else
        /* FIXME */ 
	Tcl_DStringAppend(&names, riPtr->name, -1);
#endif 
    }
