...
static int		ConfigureScale _ANSI_ARGS_((Tcl_Interp *interp,
			    Scale *scalePtr, int argc, Arg *args,
			    int flags));
...
int
Tk_ScaleCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    scalePtr->widgetCmd = Lang_CreateWidget(interp,scalePtr->tkwin, ScaleWidgetCmd, (ClientData) scalePtr, ScaleCmdDeletedProc);
    ...
    if (ConfigureScale(interp, scalePtr, argc-2, args+2, 0) != TCL_OK) {
	goto error;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,scalePtr->tkwin));
    return TCL_OK;
...
static int
ScaleWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;			/* Argument strings. */
{
	...
	if ((argc != 2) && (argc != 3)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " coords ?value?\"",          NULL);
	    goto error;
	}
	if (argc == 3) {
	    if (Tcl_GetDouble(interp, args[2], &value) != TCL_OK) {
		goto error;
	    }
	} else {
	...
	if ((argc != 2) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " get ?x y?\"",          NULL);
	    goto error;
	}
	if (argc == 2) {
	    value = scalePtr->value;
	...
	Tcl_SprintfResult(interp, scalePtr->format, value);
	...
	switch (thing) {
	    case TROUGH1:	Tcl_SetResult(interp, "trough1",TCL_STATIC);	break;
	    case SLIDER:	Tcl_SetResult(interp, "slider",TCL_STATIC);	break;
	    case TROUGH2:	Tcl_SetResult(interp, "trough2",TCL_STATIC);	break;
	}
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " set value\"",          NULL);
	    goto error;
	}
	if (Tcl_GetDouble(interp, args[2], &value) != TCL_OK) {
	    goto error;
...
static int
ConfigureScale(interp, scalePtr, argc, args, flags)
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, scalePtr->tkwin, configSpecs,
	    argc, args, (char *) scalePtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
