...
static int		ImgCmpConfigureMaster _ANSI_ARGS_((
			    CmpMaster *masterPtr, int argc, Arg *args,
			    int flags));
 ...
 */
	...
	/* ARGSUSED */
static int
ImgCmpCreate(interp, name, argc, args, typePtr, master, clientDataPtr)
				 ...
				 * it will be returned in later callbacks. */
{
    ...
    masterPtr->imageCmd = Tcl_CreateCommand(interp, name, ImgCmpCmd, (ClientData)masterPtr, ImgCmpCmdDeletedProc);
    ...
    if (ImgCmpConfigureMaster(masterPtr, argc, args, 0) != TCL_OK) {
 ...
 */
static int
ImgCmpConfigureMaster(masterPtr, argc, args, flags)
				 ...
				 * such as TK_CONFIG_ARGV_ONLY. */
{
    ...
    int i;
    for (i=0; i<argc; i++) {
	size_t length = strlen(LangString(args[i]));
	if (strncmp(LangString(args[i]), "-window", length) == 0) {
	    masterPtr->tkwin = Tk_NameToWindow(masterPtr->interp, LangString(args[i+1]),
	...
	configSpecs, argc, args, (char *) masterPtr, flags)
	!= TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
ImgCmpCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	if (argc < 3) {
	    return Tix_ArgcError(interp, argc, args, 2, 
		"type ?option value? ...");
	}
	c = LangString(args[2])[0];
	length = strlen(LangString(args[2]));

	if ((c == 'l') && (strncmp(LangString(args[2]), "line", length) == 0)) {
	    CmpLine * newLine;

	    newLine = AddNewLine(masterPtr, argc-3, args+3);
	    if (newLine == NULL) {
		return TCL_ERROR;
	    }
	}
	    ...
	    if ((c == 'b') && (strncmp(LangString(args[2]), "bitmap", length) == 0)) {
	    ...
	    else if ((c == 'i') && (strncmp(LangString(args[2]), "image", length) == 0)) {
	    ...
	    else if ((c == 's') && (strncmp(LangString(args[2]), "space", length) == 0)) {
	    ...
	    else if ((c == 't') && (strncmp(LangString(args[2]), "text", length) == 0)) {
		...
		Tcl_AppendResult(interp, "unknown option \"",
		    LangString(args[2]), "\", must be bitmap, image, line, ",
	...
	if (argc == 2) {
	    ...
	    code = ImgCmpConfigureMaster(masterPtr, argc-2, args+2,
		TK_CONFIG_ARGV_ONLY);
	}
	return code;
    } else if ((c == 'i') && (strncmp(LangString(args[1]), "itemconfigure", length)== 0)) {

    } else if ((c == 'l') && (strncmp(LangString(args[1]), "lineconfigure", length)== 0)) {
 ...
 */
CmpLine * 
AddNewLine(masterPtr, argc, args)
    CmpMaster *masterPtr;
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	lineConfigSpecs, argc, args, (char *) lPtr, 
 ...
 */
CmpBitmapItem *
AddNewBitmap(masterPtr, line, argc, args)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	bitmapConfigSpecs, argc, args, (char *) p.bitmap, 
	TK_CONFIG_ARGV_ONLY) != TCL_OK) {
 ...
 */
CmpImageItem *
AddNewImage(masterPtr, line, argc, args)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	imageConfigSpecs, argc, args, (char *) p.image, 
	TK_CONFIG_ARGV_ONLY) != TCL_OK) {
 ...
 */
CmpSpaceItem *
AddNewSpace(masterPtr, line, argc, args)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	spaceConfigSpecs, argc, args, (char *)p.space, 
	TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	goto error;
    }
 ...
 */
CmpTextItem *
AddNewText(masterPtr, line, argc, args)
    CmpMaster *masterPtr;
    CmpLine *line;
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp, masterPtr->tkwin,
	textConfigSpecs, argc, args, (char *) p.text, 
	TK_CONFIG_ARGV_ONLY) != TCL_OK) {
				 ...
				 * correspond to imageX and imageY. */
{
	...
	Tk_Fill3DRectangle(masterPtr->tkwin, drawable, 
	    ...
	    masterPtr->width  - 2*masterPtr->padX - imageX,
	    masterPtr->height - 2*masterPtr->padY - imageY,
