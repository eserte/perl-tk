...
static int		ConfigureButton _ANSI_ARGS_((Tcl_Interp *interp,
			    Button *butPtr, int argc, Arg *args,
			    int flags));
...
int
Tk_ButtonCmd(clientData, interp, argc, args)
...
{
    return ButtonCreate(clientData, interp, argc, args, TYPE_BUTTON);
}

int
Tk_CheckbuttonCmd(clientData, interp, argc, args)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    return ButtonCreate(clientData, interp, argc, args, TYPE_CHECK_BUTTON);
}

int
Tk_LabelCmd(clientData, interp, argc, args)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    return ButtonCreate(clientData, interp, argc, args, TYPE_LABEL);
}

int
Tk_RadiobuttonCmd(clientData, interp, argc, args)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    return ButtonCreate(clientData, interp, argc, args, TYPE_RADIO_BUTTON);
}

/*
 *--------------------------------------------------------------
...
static int
ButtonCreate(clientData, interp, argc, args, type)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
				 ...
				 * TYPE_RADIO_BUTTON. */
{
    ...
    butPtr->widgetCmd = Lang_CreateWidget(interp,butPtr->tkwin, ButtonWidgetCmd, (ClientData) butPtr, ButtonCmdDeletedProc);
    ...
    if (ConfigureButton(interp, butPtr, argc-2, args+2,
	    configFlags[type]) != TCL_OK) {
	Tk_DestroyWindow(butPtr->tkwin);
	return TCL_ERROR;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,butPtr->tkwin));
    return TCL_OK;
}

/*
...
static int
ButtonWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	result = Tk_ConfigureValue(interp, butPtr->tkwin, configSpecs,
		(char *) butPtr, LangString(args[2]), configFlags[butPtr->type]);
    } else if ((c == 'c') && (strncmp(LangString(args[1]), "configure", length) == 0)
	    && (length >= 2)) {
	if (argc == 2) {
	    result = Tk_ConfigureInfo(interp, butPtr->tkwin, configSpecs,
		    (char *) butPtr,          NULL, configFlags[butPtr->type]);
	} else if (argc == 3) {
	    result = Tk_ConfigureInfo(interp, butPtr->tkwin, configSpecs,
		    (char *) butPtr, LangString(args[2]),
		    configFlags[butPtr->type]);
	} else {
	    result = ConfigureButton(interp, butPtr, argc-2, args+2,
		    configFlags[butPtr->type] | TK_CONFIG_ARGV_ONLY);
	}
    } else if ((c == 'd') && (strncmp(LangString(args[1]), "deselect", length) == 0)
    ...
    } else if ((c == 'f') && (strncmp(LangString(args[1]), "flash", length) == 0)
    ...
    } else if ((c == 't') && (strncmp(LangString(args[1]), "toggle", length) == 0)
	...
	Tcl_SprintfResult(interp,
		"bad option \"%.50s\":  must be %s", LangString(args[1]),
...
static int
ConfigureButton(interp, butPtr, argc, args, flags)
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, butPtr->tkwin, configSpecs,
	    argc, args, (char *) butPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    ClientData clientData;	/* Information about widget. */
{
	...
	if (butPtr->image != NULL) {
	    if ((butPtr->selectImage != NULL) && 
	          ((butPtr->flags & SELECTED) || 
	           ((butPtr->type == TYPE_BUTTON) && (butPtr->state == tkActiveUid)))) {
