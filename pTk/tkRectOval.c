...
static int
CreateRectOval(interp, canvas, itemPtr, argc, args)
    ...
    Arg *args;			/* Arguments describing rectangle. */
{
    ...
    if (ConfigureRectOval(interp, canvas, itemPtr, argc-4, args+4, 0)
	    != TCL_OK) {
...
static int
RectOvalCoords(interp, canvas, itemPtr, argc, args)
					 ...
					 * x2, y2, ... */
{
	...
	Tcl_DoubleResults(interp, 4, 1, 
                          rectOvalPtr->bbox[0],  rectOvalPtr->bbox[1], 
                          rectOvalPtr->bbox[2],  rectOvalPtr->bbox[3]);
    } else if (argc == 4) {
	if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]),
		    &rectOvalPtr->bbox[0]) != TCL_OK)
		|| (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]),
...
static int
ConfigureRectOval(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
