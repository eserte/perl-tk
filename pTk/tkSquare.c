...
static int		SquareConfigure _ANSI_ARGS_((Tcl_Interp *interp,
			    Square *squarePtr, int argc, Arg *args,
			    int flags));
...
int
SquareCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    squarePtr->widgetCmd = Lang_CreateWidget(interp,squarePtr->tkwin, SquareWidgetCmd, (ClientData) squarePtr, SquareCmdDeletedProc);
    ...
    Arg *args;			/* Argument strings. */
{
    ...
    } else if ((c == 'p') && (strncmp(LangString(args[1]), "position", length) == 0)) {
	if ((argc != 2) && (argc != 4)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
	...
	if (argc == 4) {
	    if ((Tk_GetPixels(interp, squarePtr->tkwin, LangString(args[2]),
		    ...
		    squarePtr->tkwin, LangString(args[3]), &squarePtr->y) != TCL_OK)) {
		goto error;
	    }
	    KeepInWindow(squarePtr);
	}
	Tcl_IntResults(interp,2,0, squarePtr->x, squarePtr->y);
    } else if ((c == 's') && (strncmp(LangString(args[1]), "size", length) == 0)) {
	if ((argc != 2) && (argc != 3)) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
	...
	if (argc == 3) {
	    ...
	    if (Tk_GetPixels(interp, squarePtr->tkwin, LangString(args[2]), &i) != TCL_OK) {
		goto error;
	    }
	    if ((i <= 0) || (i > 100)) {
