 ...
 * Tk_ParseArgv --
 *
 *	Process an args array according to a table of expected
 ...
 *	Under normal conditions, both *argcPtr and LangString(args[0]) are modified
...
int
Tk_ParseArgv(interp, tkwin, argcPtr, args, argTable, flags)
    Tcl_Interp *interp;		/* Place to store error message. */
    Tk_Window tkwin;		/* Window to use for setting Tk options.
				 * NULL means ignore Tk option specs. */
    int *argcPtr;		/* Number of arguments in args.  Modified
				 * to hold # args left in args at end. */
    Arg *args;		/* Array of arguments.  Modified to hold
				 ...
				 * such as TK_ARGV_NO_DEFAULTS. */
{
    ...
    int srcIndex;		/* Location from which to read next argument
				 * from args. */
    int dstIndex;		/* Index into args to which next unused
				 * argument should be copied (never greater
				 * than srcIndex). */
    int argc;			/* # arguments in args still to process. */
    ...
    while (argc > 0) {
	curArg = LangString(args[srcIndex]);
	srcIndex++;
	argc--;
	...
	if (matchPtr == NULL) {
	    ...
	    LangSetString(args+dstIndex, curArg);
	    dstIndex++;
	    continue;
	}
	...
	switch (infoPtr->type) {
		...
		*((int *) infoPtr->dst) = (int) infoPtr->src;
		...
		if (argc == 0) {
		    ...
		    *((int *) infoPtr->dst) =
			    strtol(LangString(args[srcIndex]), &endPtr, 0);
		    if ((endPtr == LangString(args[srcIndex])) || (*endPtr != 0)) {
			Tcl_AppendResult(interp, "expected integer argument ",
				"for \"", infoPtr->key, "\" but got \"",
				LangString(args[srcIndex]), "\"",          NULL);
			return TCL_ERROR;
		    }
		    srcIndex++;
		    argc--;
		...
		if (argc == 0) {
		    ...
		    *((char **)infoPtr->dst) = LangString(args[srcIndex]);
		    srcIndex++;
		    argc--;
		}
		break;
		...
		if (argc == 0) {
		    ...
		    *((Tk_Uid *)infoPtr->dst) = Tk_GetUid(LangString(args[srcIndex]));
		    srcIndex++;
		    argc--;
		}
		break;
		...
		*((int *) infoPtr->dst) = dstIndex;
		...
		Tk_AddOption(tkwin, infoPtr->dst, LangString(args[srcIndex]),
			TK_INTERACTIVE_PRIO);
		srcIndex++;
		argc--;
		break;
		...
		Tk_AddOption(tkwin, LangString(args[srcIndex]), LangString(args[srcIndex+1]),
			TK_INTERACTIVE_PRIO);
		srcIndex += 2;
		argc -= 2;
		break;
	    default:
		Tcl_SprintfResult(interp, "bad argument type %d in Tk_ArgvInfo",
			infoPtr->type);
		return TCL_ERROR;
	}
    }
    ...
    argsDone:
    while (argc) {
	LangSetString(args+dstIndex, LangString(args[srcIndex]));
	srcIndex++;
	dstIndex++;
	argc--;
    }
    LangSetString(args+dstIndex,          NULL);
    *argcPtr = dstIndex;
    return TCL_OK;
