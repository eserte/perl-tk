...
static char sccsid[] = "@(#) tkFocus.c 1.23 95/06/05 21:46:42";
...
int
Tk_FocusCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	if (focusWinPtr != NULL) {
	    Tcl_ArgResult(interp,LangWidgetArg(interp,(Tk_Window)(focusWinPtr)));
	}
	return TCL_OK;
    }
	...
	if (LangString(args[1])[0] == '.') {
	    newPtr = (TkWindow *) Tk_NameToWindow(interp, LangString(args[1]), tkwin);
	    if (newPtr == NULL) {
		return TCL_ERROR;
	    }
	...
	newPtr = (TkWindow *) Tk_NameToWindow(interp, LangString(args[2]), tkwin);
	if (newPtr == NULL) {
	    return TCL_ERROR;
	}
	...
	for (topLevelPtr = newPtr; topLevelPtr != NULL;
		...
		for (focusPtr = newPtr->mainPtr->focusPtr; focusPtr != NULL;
			...
			Tcl_ArgResult(interp,LangWidgetArg(interp,(Tk_Window)(focusPtr->focusWinPtr)));
			return TCL_OK;
		    }
		}
		Tcl_ArgResult(interp,LangWidgetArg(interp,(Tk_Window)(topLevelPtr)));
		return TCL_OK;
	    }
	}
    } else {
	Tcl_AppendResult(interp, "bad option \"", LangString(args[1]),
		"\": must be -displayof, -force, or -lastfor",          NULL);
	return TCL_ERROR;
    }
    return TCL_OK;
}
