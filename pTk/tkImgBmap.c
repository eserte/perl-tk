...
static int		ImgBmapConfigureMaster _ANSI_ARGS_((
			    BitmapMaster *masterPtr, int argc, Arg *args,
			    int flags));
 ...
 */
	...
	/* ARGSUSED */
static int
ImgBmapCreate(interp, name, argc, args, typePtr, master, clientDataPtr)
				 ...
				 * it will be returned in later callbacks. */
{
    ...
    if (ImgBmapConfigureMaster(masterPtr, argc, args, 0) != TCL_OK) {
...
static int
ImgBmapConfigureMaster(masterPtr, argc, args, flags)
...
static int
ImgBmapCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	if (argc == 2) {
	    ...
	    code = ImgBmapConfigureMaster(masterPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
	return code;
    } else {
