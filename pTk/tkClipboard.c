 ...
 *	is left in Tcl_GetResult(interp).
 *
 * Side effects:
...
int
Tk_ClipboardCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    if ((c == 'a') && (strncmp(LangString(args[1]), "append", length) == 0)) {
	Atom target, format;
	...
	if (count > 0) {
	    Tcl_AppendResult(interp, "wrong # argp: should be \"", LangString(args[0]),
		    " clear ?options?\"",          NULL);
	    return TCL_ERROR;
	}
	if (path != NULL) {
 ...
 *	If an error occurs then an error message is left in Tcl_GetResult(interp)
