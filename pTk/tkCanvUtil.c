...
static Arg		CanvasTagsPrintProc _ANSI_ARGS_((ClientData clientData,
...
static int
CanvasTagsParseProc(clientData, interp, tkwin, arg, widgRec, offset)
    ...
    int offset;				/* Offset into item (ignored). */
{
    ...
    for (i = 0; i < argc; i++) {
	itemPtr->tagPtr[i] = Tk_GetUid(LangString(args[i]));
    }
    if (freeProc)
     (*freeProc)(argc, args);
    return TCL_OK;
