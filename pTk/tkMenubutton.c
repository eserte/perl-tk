...
typedef struct {
    ...
    Arg menuName;		/* Name of menu associated with widget.
				 * Malloc-ed. */
...
static int		ConfigureMenuButton _ANSI_ARGS_((Tcl_Interp *interp,
			    MenuButton *mbPtr, int argc, Arg *args,
			    int flags));
...
int
Tk_MenubuttonCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    mbPtr->interp = interp;
    mbPtr->widgetCmd = Lang_CreateWidget(interp,mbPtr->tkwin, MenuButtonWidgetCmd, (ClientData) mbPtr, MenuButtonCmdDeletedProc);
    ...
    if (ConfigureMenuButton(interp, mbPtr, argc-2, args+2, 0) != TCL_OK) {
	Tk_DestroyWindow(mbPtr->tkwin);
	return TCL_ERROR;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,mbPtr->tkwin));
    return TCL_OK;
}

/*
...
static int
MenuButtonWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	result = Tk_ConfigureValue(interp, mbPtr->tkwin, configSpecs,
		(char *) mbPtr, LangString(args[2]), 0);
	...
	if (argc == 2) {
	    ...
	    result = ConfigureMenuButton(interp, mbPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    } else {
...
static int
ConfigureMenuButton(interp, mbPtr, argc, args, flags)
