...
static int
CreateBitmap(interp, canvas, itemPtr, argc, args)
    ...
    Arg *args;			/* Arguments describing rectangle. */
{
    ...
    if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &bmapPtr->x) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]), &bmapPtr->y)
		!= TCL_OK)) {
	return TCL_ERROR;
    }

    if (ConfigureBitmap(interp, canvas, itemPtr, argc-2, args+2, 0) != TCL_OK) {
...
static int
BitmapCoords(interp, canvas, itemPtr, argc, args)
					 ...
					 * x2, y2, ... */
{
    ...
    if (argc == 0) {
	Tcl_DoubleResults(interp, 2, 1, bmapPtr->x, bmapPtr->y);
    } else if (argc == 2) {
	if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &bmapPtr->x) != TCL_OK)
		|| (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]), &bmapPtr->y)
		    != TCL_OK)) {
...
static int
ConfigureBitmap(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
