...
static int		ConfigureEntry _ANSI_ARGS_((Tcl_Interp *interp,
			    Entry *entryPtr, int argc, Arg *args,
			    int flags));
...
int
Tk_EntryCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    entryPtr->widgetCmd = Lang_CreateWidget(interp,entryPtr->tkwin, EntryWidgetCmd, (ClientData) entryPtr, EntryCmdDeletedProc);
    ...
    if (ConfigureEntry(interp, entryPtr, argc-2, args+2, 0) != TCL_OK) {
	goto error;
    }

    Tcl_ArgResult(interp,LangWidgetArg(interp,entryPtr->tkwin));
    return TCL_OK;
...
static int
EntryWidgetCmd(clientData, interp, argc, args)
    ...
    Arg *args;			/* Argument strings. */
{
	...
	result = Tk_ConfigureValue(interp, entryPtr->tkwin, configSpecs,
		(char *) entryPtr, LangString(args[2]), 0);
	...
	if (argc == 2) {
	    ...
	    result = ConfigureEntry(interp, entryPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " insert index text\"",
		             NULL);
	    goto error;
	}
	if (GetEntryIndex(interp, entryPtr, args[2], &index) != TCL_OK) {
	...
	if (entryPtr->state == tkNormalUid) {
	    InsertChars(entryPtr, index, LangString(args[3]));
	}
	...
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " scan mark|dragto x\"",          NULL);
	    goto error;
	}
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " select option ?index?\"",          NULL);
	    goto error;
	}
	length = strlen(LangString(args[2]));
	c = LangString(args[2])[0];
	if ((c == 'c') && (strncmp(LangString(args[2]), "clear", length) == 0)) {
	    if (argc != 3) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection clear\"",          NULL);
		goto error;
	    }
	    ...
	    goto done;
	} else if ((c == 'p') && (strncmp(LangString(args[2]), "present", length) == 0)) {
	    if (argc != 3) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection present\"",          NULL);
		goto error;
	    }
	    if (entryPtr->selectFirst == -1) {
	...
	if ((c == 'a') && (strncmp(LangString(args[2]), "adjust", length) == 0)) {
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection adjust index\"",
			         NULL);
		goto error;
	    }
	    ...
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection from index\"",
			         NULL);
		goto error;
	    }
	    entryPtr->selectAnchor = index;
	} else if ((c == 'r') && (strncmp(LangString(args[2]), "range", length) == 0)) {
	    if (argc != 5) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection range start end\"",
			         NULL);
		goto error;
	    }
	    ...
	    if (argc != 4) {
		Tcl_AppendResult(interp, "wrong # args: should be \"",
			LangString(args[0]), " selection to index\"",
			         NULL);
		goto error;
	    }
	    EntrySelectTo(entryPtr, index);
...
static int
ConfigureEntry(interp, entryPtr, argc, args, flags)
    ...
    int flags;			/* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(interp, entryPtr->tkwin, configSpecs,
	    argc, args, (char *) entryPtr, flags) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 *	error occurs then an error message is left in Tcl_GetResult(interp).
 *
 * Side effects:
 *	None.
 *
