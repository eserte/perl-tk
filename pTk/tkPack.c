...
static int		ConfigureSlaves _ANSI_ARGS_((Tcl_Interp *interp,
			    Tk_Window tkwin, int argc, Arg *args));
...
int
Tk_PackCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    if ((argc >= 2) && (LangString(args[1])[0] == '.')) {
	return ConfigureSlaves(interp, tkwin, argc-1, args+1);
    }
    if (argc < 3) {
    ...
    if ((c == 'a') && (length >= 2)
	    && (strncmp(LangString(args[1]), "after", length) == 0)) {
	Packer *prevPtr;
	Tk_Window tkwin2;

	tkwin2 = Tk_NameToWindow(interp, LangString(args[2]), tkwin);
	if (tkwin2 == NULL) {
	    return TCL_ERROR;
	}
	...
	return PackAfter(interp, prevPtr, prevPtr->masterPtr, argc-3, args+3);
    } else if ((c == 'a') && (length >= 2)
	    && (strncmp(LangString(args[1]), "append", length) == 0)) {
	Packer *masterPtr;
	...
	return PackAfter(interp, prevPtr, masterPtr, argc-3, args+3);
    } else if ((c == 'b') && (strncmp(LangString(args[1]), "before", length) == 0)) {
	...
	return ConfigureSlaves(interp, tkwin, argc-2, args+2);
	...
	for (i = 2; i < argc; i++) {
	    slave = Tk_NameToWindow(interp, LangString(args[i]), tkwin);
	    if (slave == NULL) {
		continue;
	    }
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " info window\"",          NULL);
	    return TCL_ERROR;
	}
	slave = Tk_NameToWindow(interp, LangString(args[2]), tkwin);
	if (slave == NULL) {
	    return TCL_ERROR;
	}
	slavePtr = GetPacker(slave);
    ...
    } else if ((c == 'p') && (strncmp(LangString(args[1]), "propagate", length) == 0)) {
	Tk_Window master;
	Packer *masterPtr;
	int propagate;
	...
	if (argc > 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " propagate window ?boolean?\"",          NULL);
	    return TCL_ERROR;
	}
	master = Tk_NameToWindow(interp, LangString(args[2]), tkwin);
	if (master == NULL) {
	    return TCL_ERROR;
	}
	...
	if (Tcl_GetBoolean(interp, args[3], &propagate) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (propagate) {
    ...
    } else if ((c == 's') && (strncmp(LangString(args[1]), "slaves", length) == 0)) {
	Tk_Window master;
	Packer *masterPtr, *slavePtr;

	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " slaves window\"",          NULL);
	    return TCL_ERROR;
	}
	master = Tk_NameToWindow(interp, LangString(args[2]), tkwin);
	if (master == NULL) {
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " unpack window\"",          NULL);
	    return TCL_ERROR;
	}
	tkwin2 = Tk_NameToWindow(interp, LangString(args[2]), tkwin);
	if (tkwin2 == NULL) {
...
static int
PackAfter(interp, prevPtr, masterPtr, argc, args)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
    Packer *prevPtr;		/* Pack windows in args just after this
				 * window;  NULL means pack as first
				 * child of masterPtr. */
    Packer *masterPtr;		/* Master in which to pack windows. */
    int argc;			/* Number of elements in args. */
    Arg *args;		/* Array of lists, each containing 2
				 ...
				 * against which to pack. */
{
    ...
    for ( ; argc > 0; argc -= 2, args += 2, prevPtr = packPtr) {
	if (argc < 2) {
	    Tcl_AppendResult(interp, "wrong # args: window \"",
		    LangString(args[0]), "\" should be followed by options",
		             NULL);
	    return TCL_ERROR;
	}
	...
	tkwin = Tk_NameToWindow(interp, LangString(args[0]), masterPtr->tkwin);
	if (tkwin == NULL) {
	    return TCL_ERROR;
	}
	...
	for (ancestor = masterPtr->tkwin; ; ancestor = Tk_Parent(ancestor)) {
		...
		badWindow:
		Tcl_AppendResult(interp, "can't pack ", LangString(args[0]),
 ...
 *	returned and Tcl_GetResult(interp) is set to contain an error message.
 *
 * Side effects:
...
static int
ConfigureSlaves(interp, tkwin, argc, args)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
    Tk_Window tkwin;		/* Any window in application containing
				 * slaves.  Used to look up slave names. */
    int argc;			/* Number of elements in args. */
    Arg *args;		/* Argument strings:  contains one or more
				 ...
				 * window name. */
{
	...
	if (LangString(args[numWindows])[0] != '.') {
	    break;
	}
    }
	...
	slave = Tk_NameToWindow(interp, LangString(args[j]), tkwin);
	if (slave == NULL) {
	    return TCL_ERROR;
	}
	...
	for (i = numWindows; i < argc; i+=2) {
		...
		if (j == 0) {
		    ...
		    if (prevPtr->masterPtr == NULL) {
			notPacked:
			Tcl_AppendResult(interp, "window \"", LangString(args[i+1]),
				"\" isn't packed",          NULL);
			return TCL_ERROR;
		    }
		...
		} else if (strcmp(LangString(args[i+1]), "y") == 0) {
		    slavePtr->flags = (slavePtr->flags & ~FILLX) | FILLY;
		} else if (strcmp(LangString(args[i+1]), "both") == 0) {
		    slavePtr->flags |= FILLX|FILLY;
		} else {
		    Tcl_AppendResult(interp, "bad fill style \"", LangString(args[i+1]),
		...
		if ((Tk_GetPixels(interp, slave, LangString(args[i+1]), &tmp) != TCL_OK)
		    ...
		    Tcl_AppendResult(interp, "bad pad value \"", LangString(args[i+1]),
			    "\": must be positive screen distance",
			             NULL);
		    return TCL_ERROR;
		}
		...
		if ((c == 't') && (strcmp(LangString(args[i+1]), "top") == 0)) {
		    slavePtr->side = TOP;
		} else if ((c == 'b') && (strcmp(LangString(args[i+1]), "bottom") == 0)) {
		    slavePtr->side = BOTTOM;
		} else if ((c == 'l') && (strcmp(LangString(args[i+1]), "left") == 0)) {
		    slavePtr->side = LEFT;
		} else if ((c == 'r') && (strcmp(LangString(args[i+1]), "right") == 0)) {
		    slavePtr->side = RIGHT;
		} else {
		    Tcl_AppendResult(interp, "bad side \"", LangString(args[i+1]),
