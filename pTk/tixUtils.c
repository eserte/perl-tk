 ...
 *
 */
int Tix_HandleSubCmds(cmdInfo, subCmdInfo, clientData, interp, argc, args)
    Tix_CmdInfo * cmdInfo;
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
	    LangString(args[0]), " ", cmdInfo->info, "\".",          NULL);

	return TCL_ERROR;
    }

    /*
     * Now try to match the subcommands with LangString(args[1])
     */
    argc -= 1;
	...
	if (s->name == TIX_DEFAULT_SUBCMD) {
	    if (s->checkArgvProc) {
	      if (!((*s->checkArgvProc)(clientData, interp, argc+1, args+1))) {
		    /* Some improper args in the arguments of the default
		     * subcommand
		     */
		    break;
		}
	    }
	    return (*s->proc)(clientData, interp, argc+1, args+1);
	}

	if (s->namelen == TIX_DEFAULT_LEN) {
	    s->namelen = strlen(s->name);
	}
	if (s->name[0] == LangString(args[1])[0] && strncmp(LangString(args[1]),s->name,len)==0) {
	    if (argc < s->minargc) {
		error = WRONG_ARGC;
		break;
	    }
	     ...
	     */
	    return (*s->proc)(clientData, interp, argc, args+2);
	}
    }
	 ...
	 */
	Tcl_AppendResult(interp, "wrong # args: should be \"",
	    LangString(args[0]), " ", LangString(args[1]), " ", s->info,          NULL);
    } else {
	int max;
 ...
 *
 */
int Tix_ShellInputCmd(clientData, interp, argc, args)
    ClientData clientData;	/* Main window associated with
				 * interpreter. */
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
...
#define BUFFER_SIZE 4000
    ...
    if (Tcl_GetResult(interp) != 0 && isatty(0)) {
	printf("%s\n", Tcl_GetResult(interp));
    }
