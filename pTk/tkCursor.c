 ...
 *	None is returned and an error message is left in Tcl_GetResult(interp).
				 ...
				 * for details on legal syntax. */
{
	...
	if (argc == 1) {
	    ...
	    if (XParseColor(nameKey.display, Tk_Colormap(tkwin), LangString(args[1]),
		    &fg) == 0) {
		Tcl_AppendResult(interp, "invalid color name \"", LangString(args[1]),
			"\"",          NULL);
		goto error;
	    }
	    if (argc == 2) {
		...
		if (XParseColor(nameKey.display, Tk_Colormap(tkwin), LangString(args[2]),
			&bg) == 0) {
		    Tcl_AppendResult(interp, "invalid color name \"", LangString(args[2]),
			    "\"",          NULL);
		    goto error;
		}
	    }
	...
	if (argc == 2) {
	    ...
	    if (XReadBitmapFile(nameKey.display,
		    RootWindowOfScreen(Tk_Screen(tkwin)), LangString(args[1]),
	    ...
	    if (XParseColor(nameKey.display, Tk_Colormap(tkwin), LangString(args[3]),
		    &bg) == 0) {
		Tcl_AppendResult(interp, "invalid color name \"", LangString(args[3]),
			"\"",          NULL);
		goto error;
	    }
 ...
 *	returned and an error message is left in Tcl_GetResult(interp).  The
