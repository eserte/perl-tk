...
static int		ConfigureSlave _ANSI_ARGS_((Tcl_Interp *interp,
			    Slave *slavePtr, int argc, Arg *args));
...
int
Tk_PlaceCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " option|pathName args",          NULL);
	return TCL_ERROR;
    }
    ...
    if (c == '.') {
	tkwin = Tk_NameToWindow(interp, LangString(args[1]), (Tk_Window) clientData);
	if (tkwin == NULL) {
	    return TCL_ERROR;
	}
	slavePtr = FindSlave(tkwin);
	return ConfigureSlave(interp, slavePtr, argc-2, args+2);
    }
    ...
    tkwin = Tk_NameToWindow(interp, LangString(args[2]), (Tk_Window) clientData);
    if (tkwin == NULL) {
	return TCL_ERROR;
    }
	...
	slavePtr = FindSlave(tkwin);
	return ConfigureSlave(interp, slavePtr, argc-3, args+3);
    } else if ((c == 'f') && (strncmp(LangString(args[1]), "forget", length) == 0)) {
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " forget pathName\"",          NULL);
	    return TCL_ERROR;
	}
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " info pathName\"",          NULL);
	    return TCL_ERROR;
	}
	hPtr = Tcl_FindHashEntry(&slaveTable, (char *) tkwin);
	if (hPtr == NULL) {
	...
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		    LangString(args[0]), " slaves pathName\"",          NULL);
	    return TCL_ERROR;
	}
    ...
    Arg *args;		/* String values for arguments. */
{
	...
	if (argc < 2) {
	    Tcl_AppendResult(interp, "extra option \"", LangString(args[0]),
		    "\" (option with no value?)",          NULL);
	    result = TCL_ERROR;
	    goto done;
	}
	length = strlen(LangString(args[0]));
	c = LangString(args[0])[1];
	if ((c == 'a') &&  LangCmpOpt("-anchor",LangString(args[0]),length) == 0 ) {
	    if (Tk_GetAnchor(interp, LangString(args[1]), &slavePtr->anchor) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    }
	} else if ((c == 'b') &&  LangCmpOpt("-bordermode",LangString(args[0]),length) == 0 ) {

	    c = LangString(args[1])[0];
	    length = strlen(LangString(args[1]));
	    if ((c == 'i') && (strncmp(LangString(args[1]), "ignore", length) == 0)
		    && (length >= 2)) {
		slavePtr->borderMode = BM_IGNORE;
	    } else if ((c == 'i') && (strncmp(LangString(args[1]), "inside", length) == 0)
		    && (length >= 2)) {
		slavePtr->borderMode = BM_INSIDE;
	    } else if ((c == 'o') && (strncmp(LangString(args[1]), "outside", length) == 0)) {

		slavePtr->borderMode = BM_OUTSIDE;
	    } else {
		Tcl_AppendResult(interp, "bad border mode \"", LangString(args[1]),
			"\": must be ignore, inside, or outside",
			         NULL);
		result = TCL_ERROR;
		goto done;
	    }
	} else if ((c == 'h') &&  LangCmpOpt("-height",LangString(args[0]),length) == 0 ) {
	    if (LangString(args[1])[0] == 0) {
		slavePtr->flags &= ~CHILD_HEIGHT;
	    } else {
		if (Tk_GetPixels(interp, slavePtr->tkwin, LangString(args[1]),
	...
	} else if ((c == 'i') &&  LangCmpOpt("-in",LangString(args[0]),length) == 0 ) {
	    Tk_Window tkwin;
	    Tk_Window ancestor;

	    tkwin = Tk_NameToWindow(interp, LangString(args[1]), slavePtr->tkwin);
	    if (tkwin == NULL) {
		result = TCL_ERROR;
		goto done;
	    }
	...
	} else if ((c == 'r') &&  LangCmpOpt("-relheight",LangString(args[0]),length) == 0 
		&& (length >= 5)) {
	    if (LangString(args[1])[0] == 0) {
		slavePtr->flags &= ~CHILD_REL_HEIGHT;
	    } else {
		if (Tcl_GetDouble(interp, args[1], &d) != TCL_OK) {
		    result = TCL_ERROR;
		    goto done;
		}
	...
	} else if ((c == 'r') &&  LangCmpOpt("-relwidth",LangString(args[0]),length) == 0 
		&& (length >= 5)) {
	    if (LangString(args[1])[0] == 0) {
	...
	} else if ((c == 'r') &&  LangCmpOpt("-relx",LangString(args[0]),length) == 0 
		&& (length >= 5)) {
	    if (Tcl_GetDouble(interp, args[1], &d) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    ...
	    slavePtr->relX = d;
	} else if ((c == 'r') &&  LangCmpOpt("-rely",LangString(args[0]),length) == 0 
		&& (length >= 5)) {
	    if (Tcl_GetDouble(interp, args[1], &d) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    ...
	    slavePtr->relY = d;
	} else if ((c == 'w') &&  LangCmpOpt("-width",LangString(args[0]),length) == 0 ) {
	    if (LangString(args[1])[0] == 0) {
	...
	} else if ((c == 'x') &&  LangCmpOpt("-x",LangString(args[0]),length) == 0 ) {
	    if (Tk_GetPixels(interp, slavePtr->tkwin, LangString(args[1]),
		    &slavePtr->x) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    }
	} else if ((c == 'y') &&  LangCmpOpt("-y",LangString(args[0]),length) == 0 ) {
	    if (Tk_GetPixels(interp, slavePtr->tkwin, LangString(args[1]),
		    &slavePtr->y) != TCL_OK) {
		result = TCL_ERROR;
		goto done;
	    ...
	    Tcl_AppendResult(interp, "unknown or ambiguous option \"",
		    LangString(args[0]), "\": must be -anchor, -bordermode, -height, ",
