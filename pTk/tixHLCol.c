 ...
 */
int
Tix_HLItem(clientData, interp, argc, args)
 ...
 */
static int
Tix_HLItemCGet(clientData, interp, argc, args)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tcl_GetInt(interp, args[1], &column) != TCL_OK) {
	return TCL_ERROR;
    }
    if (column >= wPtr->numColumns || column < 0) {
	Tcl_AppendResult(interp, "Column \"", LangString(args[1]), 
	    "\" does not exist",          NULL);
	return TCL_ERROR;
    }
    if (chPtr->col[column].iPtr == NULL) {
	/* ToDo: sloppy error msg */
	Tcl_AppendResult(interp, "Specified item does not exist",          NULL);
	return TCL_ERROR;
    }
    return Tk_ConfigureValue(interp, wPtr->dispData.tkwin, 
	chPtr->col[column].iPtr->base.diTypePtr->itemConfigSpecs,
	(char *)chPtr->col[column].iPtr, LangString(args[2]), 0);
}

/*----------------------------------------------------------------------
 ...
 */
static int
Tix_HLItemConfig(clientData, interp, argc, args)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
	...
	return Tix_DItemConfigure(chPtr->col[column].iPtr,
	    argc-2, args+2, TK_CONFIG_ARGV_ONLY);
    }
}

/*----------------------------------------------------------------------
 ...
 */
static int
Tix_HLItemCreate(clientData, interp, argc, args)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tix_DItemConfigure(iPtr, argc-2, args+2, 0) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 */
static int
Tix_HLItemDelete(clientData, interp, argc, args)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
 ...
 */
int
Tix_HLColumn(clientData, interp, argc, args)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
 ...
 */
static int
Tix_HLColWidth(clientData, interp, argc, args)
    ClientData clientData;
    Tcl_Interp *interp;		/* Current interpreter. */
    int argc;			/* Number of arguments. */
    Arg *args;		/* Argument strings. */
{
    ...
    if (Tcl_GetInt(interp, args[0], &column) != TCL_OK) {
	return TCL_ERROR;
    }
    if (column >= wPtr->numColumns || column < 0) {
	Tcl_AppendResult(interp, "Column \"", LangString(args[0]), 
	    "\" does not exist",          NULL);
	return TCL_ERROR;
    }
    if (argc == 1) {
	...
	if (Tk_GetPixels(interp, wPtr->dispData.tkwin, LangString(args[1]),
		 &newWidth) != TCL_OK) {
	    return TCL_ERROR;
	}
 	if (newWidth < 0) {
    ...
    else if (argc == 3 && strcmp(LangString(args[1]), "-char")==0) {
	if (LangString(args[2])[0] == '\0') {
	    newWidth = UNINITIALIZED;
	    goto setwidth;
	}
	if (Tcl_GetInt(interp, args[2], &newWidth) != TCL_OK) {
	    return TCL_ERROR;
	}
	if (newWidth < 0) {
	    newWidth = 0;
	...
	return Tix_ArgcError(interp, argc+3, args-3, 3,
	    "column ?-char? ?size?");
    }
