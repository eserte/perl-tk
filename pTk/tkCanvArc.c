...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_BITMAP, "-outlinestipple",          NULL,          NULL,
	         NULL, Tk_Offset(ArcItem, outlineStipple), TK_CONFIG_NULL_OK},
 ...
 *	Tcl_GetResult(interp);  in this case itemPtr is
...
static int
CreateArc(interp, canvas, itemPtr, argc, args)
    Tcl_Interp *interp;			/* Interpreter for error reporting. */
    Tk_Canvas canvas;			/* Canvas to hold new item. */
    Tk_Item *itemPtr;			/* Record to hold new item;  header
					 * has been initialized by caller. */
    int argc;				/* Number of arguments in args. */
    Arg *args;			/* Arguments describing arc. */
{
    ...
    if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &arcPtr->bbox[0]) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]),
		&arcPtr->bbox[1]) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[2]),
		    &arcPtr->bbox[2]) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[3]),
		    &arcPtr->bbox[3]) != TCL_OK)) {
	return TCL_ERROR;
    }

    if (ConfigureArc(interp, canvas, itemPtr, argc-4, args+4, 0) != TCL_OK) {
...
static int
ArcCoords(interp, canvas, itemPtr, argc, args)
					 ...
					 * x2, y2, ... */
{
    ...
    if (argc == 0) {
	Tcl_DoubleResults(interp, 4, 1, 
                          arcPtr->bbox[0], arcPtr->bbox[1],
                          arcPtr->bbox[2], arcPtr->bbox[3]);
    } else if (argc == 4) {
	if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]),
...
static int
ConfigureArc(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
