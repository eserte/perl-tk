...
static int		MwmProtocol _ANSI_ARGS_((Tcl_Interp * interp,
			    Tix_MwmInfo * wmPtr, int argc, Arg *args));
...
/* ARGSUSED */
int
Tix_MwmCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " option pathname ?arg ...?\"",          NULL);
	return TCL_ERROR;
    }
    c = LangString(args[1])[0];
    length = strlen(LangString(args[1]));

    if (!(winPtr = (TkWindow *) Tk_NameToWindow(interp, LangString(args[2]), tkwin))) {
	return TCL_ERROR;
    }
    if (!Tk_IsTopLevel(winPtr)) {
	Tcl_AppendResult(interp, LangString(args[2]), " is not a toplevel window.", NULL);
	return TCL_ERROR;
    }
    ...
    if ((c == 'd') && (strncmp(LangString(args[1]), "decorations", length) == 0)) {
	return SetMwmDecorations(interp, wmPtr, argc-3, args+3);
    }
    else if ((c == 'i') && (strncmp(LangString(args[1]), "ismwmrunning", length) == 0)) {
	...
	return MwmProtocol(interp, wmPtr, argc-3, args+3);
    }
    else if ((c == 't') && (strncmp(LangString(args[1]), "transientfor", length) == 0)) {
	return SetMwmTransientFor(interp, wmPtr, winPtr, argc-3, args+3);
    }
    else {
	Tcl_AppendResult(interp, "unknown or ambiguous option \"",
	    LangString(args[1]), "\": must be decorations, ismwmrunning, protocol ",
	    "or transientfor",
	    NULL);
	return TCL_ERROR;
    }
 ...
 */
static
int SetMwmDecorations(interp, wmPtr, argc, args)
    Tcl_Interp *interp;
    Tix_MwmInfo*wmPtr;
    int 	argc;
    Arg *args;
{
	...
	if (argc == 0) {
	     ...
	     * Query only one hint
	     */
	    if ((decor = MwmDecor(interp, LangString(args[0]))) == MWM_DECOR_UNKNOWN) {
		return TCL_ERROR;
	    }
	...
	for (i=0; i<argc; i+=2) {
	    ...
	    if ((decor = MwmDecor(interp, LangString(args[i]))) == MWM_DECOR_UNKNOWN)  {
		return TCL_ERROR;
	    }

	    if (Tcl_GetBoolean(interp, args[i+1], &value) != TCL_OK) {
		return TCL_ERROR;
	    }
...
static int MwmProtocol(interp, wmPtr, argc, args)
    Tcl_Interp * interp;
    Tix_MwmInfo * wmPtr;
    int argc;
    Arg *args;
{
    ...
    if (strncmp(LangString(args[0]), "add", len) == 0 && argc == 3) {
	AddMwmProtocol(interp, wmPtr, LangString(args[1]), LangString(args[2]));
    }
    else if (strncmp(LangString(args[0]), "activate", len) == 0 && argc == 2) {
	ActivateMwmProtocol(interp, wmPtr, LangString(args[1]));
    }
    else if (strncmp(LangString(args[0]), "deactivate", len) == 0 && argc == 2) {
	DeactivateMwmProtocol(interp, wmPtr, LangString(args[1]));
    }
    else if (strncmp(LangString(args[0]), "delete", len) == 0 && argc == 2) {
	DeleteMwmProtocol(interp, wmPtr, LangString(args[1]));
    }
    else {
    ...
    ClientData clientData;
{
	...
	Tcl_VarEval(wmPtr->interp, "wm protocol ", Tk_PathName(wmPtr->tkwin),
	    " _MOTIF_WM_MESSAGES {;}", NULL);
	wmPtr->addedMwmMsg = 1;
    }
#endif
...
static
int SetMwmTransientFor(interp, wmPtr, mainWindow, argc, args)
    Tcl_Interp *interp;
    Tix_MwmInfo*wmPtr;
    TkWindow   *mainWindow;
    int 	argc;
    Arg *args;
{
	...
	master = (TkWindow *) Tk_NameToWindow(interp, LangString(args[0]), 
    ...
    Tk_Window tkwin;
{
    ...
    if (!inited) {
	Tcl_InitHashTable(&mwmTable, sizeof(Tk_Window)/sizeof(int));
	inited = 1;
    }
	...
	Tcl_InitHashTable(&wmPtr->protocols, sizeof(Atom)/sizeof(int));
