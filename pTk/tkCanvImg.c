...
static int
CreateImage(interp, canvas, itemPtr, argc, args)
    ...
    Arg *args;			/* Arguments describing rectangle. */
{
    ...
    if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &imgPtr->x) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, canvas, LangString(args[1]), &imgPtr->y)
		!= TCL_OK)) {
	return TCL_ERROR;
    }

    if (ConfigureImage(interp, canvas, itemPtr, argc-2, args+2, 0) != TCL_OK) {
...
static int
ImageCoords(interp, canvas, itemPtr, argc, args)
					 ...
					 * x2, y2, ... */
{
    ...
    if (argc == 0) {
	Tcl_DoubleResults(interp, 2, 1, imgPtr->x, imgPtr->y); 
    } else if (argc == 2) {
	if ((Tk_CanvasGetCoord(interp, canvas, LangString(args[0]), &imgPtr->x) != TCL_OK)
...
static int
ConfigureImage(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
