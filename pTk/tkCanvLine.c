...
static Tk_ConfigSpec configSpecs[] = {
    ...
    {TK_CONFIG_BOOLEAN, "-smooth",          NULL,          NULL,
    ...
    {TK_CONFIG_BITMAP, "-stipple",          NULL,          NULL,
	         NULL, Tk_Offset(LineItem, fillStipple), TK_CONFIG_NULL_OK},
...
static int
CreateLine(interp, canvas, itemPtr, argc, args)
    Tcl_Interp *interp;			/* Interpreter for error reporting. */
    Tk_Canvas canvas;			/* Canvas to hold new item. */
    Tk_Item *itemPtr;			/* Record to hold new item;  header
					 * has been initialized by caller. */
    int argc;				/* Number of arguments in args. */
    Arg *args;			/* Arguments describing line. */
{
	...
	if ((!isdigit(UCHAR(LangString(args[i])[0]))) &&
		((LangString(args[i])[0] != '-')
		|| ((LangString(args[i])[1] != '.') && !isdigit(UCHAR(LangString(args[i])[1]))))) {
	    break;
	}
    }
    if (LineCoords(interp, canvas, itemPtr, i, args) != TCL_OK) {
	goto error;
    }
    if (ConfigureLine(interp, canvas, itemPtr, argc-i, args+i, 0) == TCL_OK) {
	return TCL_OK;
    }
...
static int
LineCoords(interp, canvas, itemPtr, argc, args)
					 ...
					 * x2, y2, ... */
{
	...
	for (i = 0; i < numCoords; i++, coordPtr++) {
	    ...
	    Tcl_DoubleResults(interp, 1, 1, *coordPtr);
	}
    } else if (argc < 4) {
	...
	for (i = argc-1; i >= 0; i--) {
	    if (Tk_CanvasGetCoord(interp, canvas, LangString(args[i]),
...
static int
ConfigureLine(interp, canvas, itemPtr, argc, args, flags)
    Tcl_Interp *interp;		/* Used for error reporting. */
				 ...
				 * record. */
{
    ...
    if (Lang_SplitList(interp, value, &argc, &args, &freeProc) != TCL_OK) {
	syntaxError:
	Tcl_ResetResult(interp);
	Tcl_AppendResult(interp, "bad arrow shape \"", value,
		"\": must be list with three numbers",          NULL);
	if (args != NULL && freeProc) {
	    (*freeProc)(argc,args);
	}
	return TCL_ERROR;
    }
    ...
    if ((Tk_CanvasGetCoord(interp, linePtr->canvas, LangString(args[0]), &a) != TCL_OK)
	    || (Tk_CanvasGetCoord(interp, linePtr->canvas, LangString(args[1]), &b)
		!= TCL_OK)
	    || (Tk_CanvasGetCoord(interp, linePtr->canvas, LangString(args[2]), &c)
		!= TCL_OK)) {
	goto syntaxError;
    }
...
{
    LineItem *linePtr = (LineItem *) recordPtr;
    Arg *args = LangAllocVec(3);
    Arg result = NULL;
    LangSetDouble(args+0,linePtr->arrowShapeA); 
    LangSetDouble(args+1,linePtr->arrowShapeB); 
    LangSetDouble(args+2,linePtr->arrowShapeC); 
    result = Tcl_Merge(3, args);
    LangFreeVec(3,args);
