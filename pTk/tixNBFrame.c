 ...
 */
int
Tix_NoteBookFrameCmd(clientData, interp, argc, args)
    ...
    Arg *args;		/* Argument strings. */
{
    ...
    Tcl_ArgResult(interp,LangWidgetArg(interp,wPtr->tkwin));
    return TCL_OK;
}

/*
    ...
    Arg *args;			/* Argument strings. */
{
	...
	((c == 'f') && (strncmp(LangString(args[1]), "focus", length) == 0))){
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " ", LangString(args[1]), " name\"",          NULL);
	    result = TCL_ERROR;
	}
	else {
	    if (strcmp(LangString(args[2]), "")==0) {
		if (c == 'a') {
		    wPtr->active = 0;
		    wPtr->focus  = 0;
		}
		...
		for (tPtr=wPtr->tabHead; tPtr; tPtr=tPtr->next) {
		    if (strcmp(LangString(args[2]), tPtr->name) == 0) {
			if (c == 'a') {
			    wPtr->active = tPtr;
			    wPtr->focus  = tPtr;
			}
	...
	if (argc < 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be ",
		LangString(args[0]), " add name ?options?",          NULL);
	    result = TCL_ERROR;
	}
	else {
	    if (AddTab(wPtr, LangString(args[2]), args+3, argc-3)!= TCL_OK) {
		result = TCL_ERROR;
	    } else {
	...
	if (argc == 3) {
		...
		(char *)wPtr, LangString(args[2]), 0);
	} else {
	    result = Tix_ArgcError(interp, argc, args, 2, "option");
	}
    }
	...
	if (argc == 2) {
	    ...
	    result = WidgetConfigure(interp, wPtr, argc-2, args+2,
		    TK_CONFIG_ARGV_ONLY);
	}
    }
	...
	if (argc != 3) {
	    result = Tix_ArgcError(interp, argc, args, 2, "page");
	    goto error;
	}

	/* Find the tab from the list */
	for (prev=tPtr=wPtr->tabHead; tPtr; prev=tPtr,tPtr=tPtr->next) {
	    if (strcmp(tPtr->name, LangString(args[2]))==0) {
		break;
	    }
	}
	if (tPtr == NULL) {
	    Tcl_AppendResult(wPtr->interp, 
		"Unknown tab \"", LangString(args[2]), "\"",          NULL);
	    goto error;
	}
	if (tPtr == prev) {
    ...
    else if ((c == 'g') && (strncmp(LangString(args[1]), "geometryinfo", length) == 0)) {
	char buff[20];
    ...
    else if ((c == 'i') && (strncmp(LangString(args[1]), "identify", length) == 0)) {
	if (argc != 4) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
		LangString(args[0]), " identify x y\"",          NULL);
	    goto error;
	}
	else {
	    int x, y, left, right;
		...
		for (tPtr=wPtr->tabHead; tPtr; tPtr=tPtr->next) {
		    ...
		    if (x >= left && x <= right && tPtr->state == tkNormalUid){
    ...
    else if ((c == 'i') && (strncmp(LangString(args[1]), "info", length) == 0)) {
	Tcl_ResetResult(interp);

	if (argc == 3 && strcmp(LangString(args[2]), "pages")==0 ) {
	    Tab * tPtr;
	...
	else if (argc == 3 && strcmp(LangString(args[2]), "active")==0 ) {
	    if (wPtr->active) {
	...
	else if (argc == 3 && strcmp(LangString(args[2]), "focus")==0 ) {
	    if (wPtr->focus) {
	...
	else if (argc == 3 && strcmp(LangString(args[2]), "focusnext")==0 ) {
	    Tab * next;
	    if (wPtr->focus) {
		if (wPtr->focus->next) {
	...
	else if (argc == 3 && strcmp(LangString(args[2]), "focusprev")==0 ) {
	    Tab * prev, *tPtr;
    ...
    else if ((c == 'p') && (strncmp(LangString(args[1]), "pagecget", length) == 0)) {
	Tab * tPtr;
	...
	if ((tPtr=FindTab(interp, wPtr, LangString(args[2]))) == NULL) {
	    goto error;
	}
	result = Tk_ConfigureValue(interp, wPtr->tkwin, tabConfigSpecs,
	    (char *)tPtr, LangString(args[3]), 0);
    }
    else if ((c == 'p') && (strncmp(LangString(args[1]), "pageconfigure", length) == 0)) {
	Tab * tPtr;

	if (argc < 3) {
	    result =  Tix_ArgcError(interp, argc, args, 2, 
		"page ?option value ...?");
	    goto error;
	}
	if ((tPtr=FindTab(interp, wPtr, LangString(args[2]))) == NULL) {
	...
	if (argc == 3) {
	    ...
	    result = TabConfigure(wPtr, tPtr, args+3, argc-3);
	}
    }
    else {
...
static int TabConfigure(wPtr, tPtr, args, argc)
    WidgetPtr wPtr;
    Tab *tPtr;
    Arg *args;
    int argc;
{
	...
	argc, args, (char *)tPtr, TK_CONFIG_ARGV_ONLY) != TCL_OK) {
	return TCL_ERROR;
    }
    ...
    int argc;
{
    ...
    if (TabConfigure(wPtr, tPtr, args, argc) != TCL_OK) {
	return TCL_ERROR;
    }
 ...
 *----------------------------------------------------------------------
 */
static void DrawTab(wPtr, tPtr, x, isActive, pixmap)
    WidgetPtr wPtr;
    Tab * tPtr;
    int x;
    int isActive;
    Pixmap pixmap;
{
    ...
    Tk_Fill3DPolygon(wPtr->tkwin, pixmap,
	border, points, NUM_TAB_POINTS,
	wPtr->borderWidth, TK_RELIEF_SUNKEN);

    if (tPtr->text != NULL) {
	if (tPtr->state == tkNormalUid) {
	    TkDisplayText(wPtr->display, pixmap, wPtr->fontPtr,
	...
	XCopyPlane(wPtr->display, tPtr->bitmap, pixmap,
	...
	XDrawLine(Tk_Display(wPtr->tkwin), pixmap, wPtr->focusGC,
            drawX,
	    drawY + tPtr->height + 1, 
	    drawX + tPtr->width,
	    drawY + tPtr->height + 1);
 ...
 *----------------------------------------------------------------------
 */
static void FocusTab(wPtr, tPtr, x, pixmap)
    WidgetPtr wPtr;
    Tab * tPtr;
    int x;
    Pixmap pixmap;
{
    ...
    GetTabPoints(wPtr, tPtr, x, points);
    Tk_Draw3DPolygon(wPtr->tkwin, pixmap,
    ...
    ClientData clientData;			/* Information about widget. */
{
	 ...
	 */
	pixmap = Tk_GetPixmap(wPtr->display, Tk_WindowId(wPtr->tkwin),
	    Tk_Width(wPtr->tkwin), Tk_Height(wPtr->tkwin),
	    Tk_Depth(wPtr->tkwin));
	XFillRectangle(Tk_Display(wPtr->tkwin), pixmap, wPtr->backPageGC,
	    0, 0, Tk_Width(wPtr->tkwin), Tk_Height(wPtr->tkwin));

	Tk_Fill3DRectangle(wPtr->tkwin, pixmap,
	...
	for (tPtr=wPtr->tabHead; tPtr; tPtr=tPtr->next) {
	    if (tPtr == wPtr->active) {
		activex = x;
		DrawTab(wPtr, tPtr, x, 1, pixmap);
	    }
	    else {
		DrawTab(wPtr, tPtr, x, 0, pixmap);
	    }
	    if (tPtr == wPtr->focus && wPtr->gotFocus) {
		FocusTab(wPtr, tPtr, x, pixmap);
	    }
	...
	/* Draw the box */
	Tk_Draw3DRectangle(wPtr->tkwin, pixmap,
		wPtr->bgBorder, 0, wPtr->tabsHeight,
		Tk_Width(wPtr->tkwin), 
		Tk_Height(wPtr->tkwin) - wPtr->tabsHeight,
		wPtr->borderWidth, wPtr->relief);
	...
	if (wPtr->active != NULL) {
	    ...
	    XFillRectangle(wPtr->display, pixmap,
		Tk_3DBorderGC(wPtr->tkwin, wPtr->bgBorder, TK_3D_FLAT_GC),
		x, y, width, height);
	}

	XCopyArea(wPtr->display, pixmap, Tk_WindowId(wPtr->tkwin),
	    wPtr->textGC, 0, 0, Tk_Width(wPtr->tkwin), Tk_Height(wPtr->tkwin),
	    0, 0);
	Tk_FreePixmap(wPtr->display, pixmap);
    }
