 ...
 *
 *	Since many Tix widgets use the same type
 *	of display items, for example, text items, image items, or text-image
 *	items (used in HList, TList and Table), it makes sense to provide
 *	a set of common routines to support these display items. Code re-use
 *	is the major issue: we don't want to re-define almost the same
 *	configSpecs again and again in different widgets. Therefore, all
 *	display items provide common methods to configure, display, calculate
 *	geometry, etc. *	
 *
 */
...
static Arg DItemPrintProc _ANSI_ARGS_((
 ...
 *----------------------------------------------------------------------
 */
int Tix_DItemConfigure(iPtr, argc, args, flags)
    Tix_DItem * iPtr;
    int argc;
    Arg *args;
    int flags;
{
    return iPtr->base.diTypePtr->configureProc(iPtr, argc, args, flags);
}
    ...
    Tix_ArgumentList *argListPtr;
{
	...
	ckfree((char*)argListPtr->arg[i].args);
    }
 ...
 */
int
Tix_SplitConfig(interp, tkwin, specsList, numLists, argc, args, argListPtr)
    Tcl_Interp * interp;
    Tk_Window tkwin;
    ...
    Tix_ArgumentList * argListPtr;
{
	...
	arg[i].argc = 0;
	arg[i].args = (Arg *)ckalloc(argc * sizeof(char*));
    }

    /* Split the arguments for the appropriate objects */
    for (n=0; n<argc; n+=2) {
	len = strlen(LangString(args[n]));
	found = 0;
	...
	for (i=0; i<numLists; i++) {
		...
		if (strncmp(LangString(args[n]), specPtr->argvName, len) == 0) {
	...
	if (found == 0) {
	    Tcl_AppendResult(interp, "unknown option \"", LangString(args[n]),
		    "\"",          NULL);
	    code = TCL_ERROR;
	    goto done;
	}
...
Tix_WidgetConfigure2(interp, tkwin, entRec, entConfigSpecs, iPtr,
		     argc, args, flags, forced, sizeChanged_ret)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
    Tk_Window tkwin;		/* Window corresponding to widgRec. */
    char * entRec;
    Tk_ConfigSpec *entConfigSpecs; /* Describes legal options of the entry */
    ...
    int * sizeChanged_ret;
{
	...
	if (Tk_ConfigureWidget(interp, tkwin,
	    entConfigSpecs, argList.arg[0].argc, argList.arg[0].args,
	...
	if (Tix_DItemConfigure(iPtr, argList.arg[1].argc,
	    argList.arg[1].args, flags) != TCL_OK) {
	    code = TCL_ERROR;
	    goto done;
	}
